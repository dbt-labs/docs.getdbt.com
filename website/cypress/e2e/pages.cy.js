...
describe('docs.getdbt.com', () => {
  ...
  sizes.forEach((size) => {
    it('verifies all the button links work and go to the correct page', () => {
      ...
      cy.url().should('eq', 'https://docs.getdbt.com/docs/introduction')
      ...
      cy.url().should('eq', 'https://docs.getdbt.com/guides/getting-started')
      cy.get('body').should('not.contain', 'Page Not Found')
      cy.go('back')
      cy.url().should('eq', 'https://docs.getdbt.com/')
      ...
      cy.url().should('eq', 'https://docs.getdbt.com/reference/dbt_project.yml')
      cy.get('body').should('not.contain', 'Page Not Found')
      cy.go('back')
      cy.url().should('eq', 'https://docs.getdbt.com/')
      cy.get(':nth-child(2) > :nth-child(2) > :nth-child(3) > .card > .card__footer > .button').click()
      cy.url().should('eq', 'https://docs.getdbt.com/docs/faqs')
      cy.get('body').should('not.contain', 'Page Not Found')
      cy.go('back')
      cy.url().should('eq', 'https://docs.getdbt.com/')
      cy.get(':nth-child(4) > :nth-child(1) > .card > .card__footer > .button').click()
      cy.url().should('eq', 'https://docs.getdbt.com/docs/dbt-cloud/cloud-overview')
      cy.get('body').should('not.contain', 'Page Not Found')
      cy.go('back')
      cy.url().should('eq', 'https://docs.getdbt.com/')
      cy.get(':nth-child(4) > :nth-child(2) > .card > .card__footer > .button').click()
      cy.url().should('eq', 'https://docs.getdbt.com/docs/dbt-cloud/dbt-cloud-api/cloud-apis')
      cy.get('body').should('not.contain', 'Page Not Found')
      cy.go('back')
      cy.url().should('eq', 'https://docs.getdbt.com/')
      ...
      cy.url().should('eq', 'https://docs.getdbt.com/faqs/project/example-projects')
      cy.get('body').should('not.contain', 'Page Not Found')
    })
  })
})