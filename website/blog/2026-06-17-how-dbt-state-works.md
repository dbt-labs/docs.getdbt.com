...
dbt State provides more speed, reduced cost, and less complexity all in one. It works wherever you do: dbt Core or <Constant name="fusion" />, locally or on any orchestrator you choose. Since it's backed by a new paid service (with a 30 day free trial), you don't need to adopt the whole dbt platform to get these savings. dbt State supersedes state-aware orchestration, which required a dbt platform subscription and moving to <Constant name="fusion" />.
