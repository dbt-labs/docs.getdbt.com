...
*You can follow along with Donny's data modeling technique for identity resolution in [this dbt project repo](https://github.com/dflynn20/jaffle_gaggle). It includes a set of demo CSV files, which you can use as [dbt seeds](/docs/building-a-dbt-project/seeds) to test Donny's project for yourself.*
...
Jaffle Shop is a demo repo referenced in [dbt’s Getting Started tutorial](/tutorial/setting-up), and its jaffles hold a special place in the dbt community’s hearts, as well as on Data Twitter™.
...
We defined the email domain extraction as a [macro](/docs/building-a-dbt-project/jinja-macros) called [`extract_email_domain`](https://github.com/dflynn20/jaffle_gaggle/blob/main/macros/extract_email_domain.sql), which we call in line 18 (which you can find in the pullout below).
