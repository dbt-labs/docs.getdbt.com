...
- A baseline state to compare your changes against. See [How it works](#how-it-works) to choose between [automatic deferral](docs/platform/about-platform-develop-defer) or [`manifest.json`](/reference/artifacts/manifest-json?version=2.0) manual setup.
...
<Expandable alt_header="Do I need to run dbt build every time I make a change?">
  ...
  If the model hasn’t been built yet in your deferred environment, the comparison can’t run. For more details, see [Defer to production](docs/platform/about-platform-develop-defer).
