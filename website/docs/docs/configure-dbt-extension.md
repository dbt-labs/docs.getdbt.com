...
4. Add a `dbt_cloud.yml` file from the <Constant name="dbt_platform" /> Account settings:
   ...
   - Place the downloaded file in your `~/.dbt/` directory. This registers and connects the extension to <Constant name="dbt_platform" /> and enables platform features such as <Constant name="mesh" /> and deferral. See [`dbt_cloud.yml`](/reference/dbt_cloud.yml) for the file's structure and required fields.
