...
<Constant name="dbt" /> <Constant name="explorer" /> now offers column-level lineage (CLL) for the resources in your <Constant name="dbt" /> project. Analytics engineers can quickly and easily gain insight into the provenance of their data products at a more granular level. For each column in a resource (model, source, or snapshot) in a <Constant name="dbt" /> project, <Constant name="explorer" /> provides end-to-end lineage for the data in that column given how it's used.
...
Refer to the following CLL caveats or limitations as you navigate <Constant name="dbt" /> <Constant name="explorer" />.
