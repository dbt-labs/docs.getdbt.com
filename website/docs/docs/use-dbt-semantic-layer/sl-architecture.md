...
At a high level, the dbt Semantic Layer enables you to define metrics and query them in various interfaces. The Semantic Layer does the heavy lifting to find where the queried data exists in your data platform and generates the SQL to make the request (including performing joins). This functionality is enabled with the following components:
...
| **Semantic Layer APIs** |  The interfaces that allow users to submit metric queries -- for example: the MetricFlow CLI, JDBC API, and GraphQL. These APIs are also the mechanisms in which tools can build first-class integrations to the dbt Semantic Layer| ❌ | ✅ | ✅ | Proprietary, Cloud (Team & Enterprise)|
