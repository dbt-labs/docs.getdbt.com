...
- We can support calculated fields for creating parameter filters or dynamically selecting metrics and dimensions. However, other uses of calculated fields are not supported. _Note: For calculated fields use cases that are not currently covered, please reach out to [dbt Support](mailto:support@getdbt.com) and share them so we can further understand._
