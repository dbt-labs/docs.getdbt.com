...
If a model is configured with `catalog_name`, dbt will use the current project's [catalog definition](/docs/mesh/about-catalogs) (in `catalogs.yml`), for the current active adapter, to resolve the top-level namespace of that model.
...
2. `project` dependencies (dbt platform Enterprise; the advantages of this approach are described [here](https://docs.getdbt.com/docs/mesh/govern/project-dependencies#advantages))
