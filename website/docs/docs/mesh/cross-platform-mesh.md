...
If a model is configured with `catalog_name`, dbt will use the current project's [catalog definition](/docs/mesh/iceberg/about-catalogs) (in `catalogs.yml`), for the current active adapter, to resolve the top-level namespace of that model.
