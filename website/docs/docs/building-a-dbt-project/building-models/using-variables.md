...
dbt provides a mechanism, [variables](dbt-jinja-functions/var), to provide data to <Term id="model">models</Term> for
...
To use a variable in a <Term id="model" />, hook, or macro, use the `{{ var('...') }}` function. More information on the `var` function can be found [here](dbt-jinja-functions/var).
