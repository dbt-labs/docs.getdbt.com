...
You can [connect](/docs/connect-adapters) to adapters and data platforms either directly in the dbt Cloud user interface (UI) or install them manually using the command line (CLI). There are two types of adapters available: 
...
To evaluate quality and maintenance, we recommend you consider their verification status. Here are the following are **Verified adapters** you can connect to:
