...
Welcome to the 2021 changelog for the <Constant name="cloud" /> application! You can use this changelog to see highlights of what was new, fixed, and enhanced.

## dbt Cloud v1.1.41 (December 8, 2021)

It's one of the best weeks of the year - it's the [dbt Summit](https://coalesce.getdbt.com/)! We'll have some exciting product announcements to share! Did somebody say [metrics](https://coalesce.getdbt.com/talks/keynote-metric-system/) and [<Constant name="core" /> v1.0](https://coalesce.getdbt.com/talks/dbt-v10-reveal/)?!
...
- Now whenever you log back into <Constant name="cloud" />, you'll return to the account and project that you most recently were working in!
...
- Some Github users were experiencing a broken profile image in <Constant name="cloud" />. This should be fixed if users disconnect and reconnect their Github accounts.
...
We’ve improved the tabbing experience in the <Constant name="cloud_ide" />. Tabs now work much more intuitively, and you don’t have to worry about losing your work anymore!
...
#### Performance improvements and enhancements
- We've been working on some nice improvements to tabs in our <Constant name="cloud_ide" />. We’ve fixed deficiencies with tabs that caused users to lose work if they didn’t hit save regularly enough. Additionally, opening, closing, and the order of the tabs work much more smoothly.
...
We shipped a resizable folder pane in the <Constant name="cloud_ide" />, and we're hearing great things! "My quality of life has greatly increased with this little update!" Hope this helps everyone else enjoy the <Constant name="cloud_ide" /> a little more too.
...
#### New products and features
- Add support for latest Core release candidates to <Constant name="cloud" />: v0.19.2-rc2 and v0.20.0-rc1

#### Bug fixes
- Add a safeguard for the SSO reauth page to avoid 401 interceptors

#### Performance improvements and enhancements
- Ensure navigation bar is in dark mode when <Constant name="cloud_ide" /> is set to dark mode
...
- Unsaved changes in the <Constant name="cloud_ide" /> are now saved when committing work.
...
A lot of improvements coming for GitLab webhooks and native auth. We also fixed a number of bugs in the <Constant name="cloud_ide" />. Our goal is for you to never see an infinite spinner again!
...
- Disable the "Restart <Constant name="cloud_ide" />" Button while the <Constant name="cloud_ide" /> is loading
...
- API Gateway is no longer flooded with errors due to <Constant name="cloud_ide" /> blindly polling dead Develop pod
...
Rolling out a few long-term bets to ensure that our beloved <Constant name="cloud" /> does not fall over for want of memory, as well as a grip of bug fixes and error messaging improvements (error messages should be helpful, not scolding or baffling, after all!)
...
This changelog wraps up work on what we've been calling the SQL Drawer in the <Constant name="cloud_ide" /> - some design nudges, some interface adjustments, overall a cleaner and snappier experience. If you haven't dipped into the <Constant name="cloud_ide" /> in a while it's worth taking a look! Some back-end work as well, making SSO and role based admin easier and more broadly available for Enterprise level folks, along with your usual assortment of bug squashes and iterations.


#### Enhancements

- Styling and copy adjustments in the Cloud <Constant name="cloud_ide" />
...
- Fixed endlessly running rpc queries with non-working cancel button on <Constant name="cloud_ide" /> refresh


## dbt Cloud v1.1.18 (January 20, 2021)

Most notable things here are around foundational work toward future feature releases, as well as strong assurances of future stability for <Constant name="cloud" />, and ensuring future sales tax compliance (which we understand turns out to be quite important!) - turns out to be a quite future-looking release!
...
- Fix <Constant name="cloud_ide" /> git actions causing open tab contents to be lost on <Constant name="cloud_ide" /> re-entry
