...
- **Correctly reference entities in filters using the object syntax** &mdash; Resolved an error where referencing an entity in a filter using the object syntax would fail. i.e {{Entity('entity_name')}} would fail to resolve.
