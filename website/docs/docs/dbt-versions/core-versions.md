...
<Constant name="dbt" /> Labs provides different support levels for different versions, which may include new features, bug fixes, or security patches:
...
- To learn about installing <Constant name="core" />, see "[How to install <Constant name="core" />](/docs/core/installation-overview)."
...
We recommend upgrading to a newer version in [<Constant name="cloud" />](/docs/dbt-versions/upgrade-dbt-version-in-cloud) or [<Constant name="core" />](/docs/core/installation-overview#upgrading-dbt-core) to continue receiving support. 
...
- You can use it in <Constant name="cloud" />. For more on <Constant name="cloud" /> versioning, see [Choosing a <Constant name="dbt" /> version](/docs/dbt-versions/upgrade-dbt-version-in-cloud).
...
Because of our new version practice, we've outlined best practices and expectations for <Constant name="dbt" /> users to upgrade as we continue to release new versions of <Constant name="core" />.
...
We are committed to avoiding breaking changes in minor versions for end users of <Constant name="dbt" />. There are two types of breaking changes that may be included in minor versions:
