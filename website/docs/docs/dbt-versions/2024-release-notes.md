...
- **Behavior change:** Set [`state_modified_compare_more_unrendered_values`](/reference/global-configs/behavior-flag-introductions#source-definitions-for-statemodified) to true to reduce false positives for `state:modified` when configs differ between `dev` and `prod` environments.
