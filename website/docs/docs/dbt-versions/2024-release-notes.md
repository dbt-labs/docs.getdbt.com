...
- **New**: [Downstream exposures](/docs/cloud-integrations/downstream-exposures-tableau) are now generally available to <Constant name="cloud" /> Enterprise plans. Downstream exposures integrate natively with Tableau (Power BI coming soon) and auto-generate downstream lineage in dbt Explorer for a richer experience.
...
- **Enhancement:** We've now introduced a new **Prune branches** [<Constant name="git" /> button](/docs/cloud/dbt-cloud-ide/ide-user-interface#prune-branches-modal) in the IDE. This button allows you to delete local branches that have been deleted from the remote repository, keeping your branch management tidy. Available in all regions now and will be released to single tenant accounts during the next release cycle.
...
- **Enhancement:** As part of our ongoing commitment to improving the [IDE](/docs/cloud/dbt-cloud-ide/develop-in-the-cloud#considerations), the filesystem now comes with improvements to speed up dbt development, such as introducing a <Constant name="git" /> repository limit of 10GB.
