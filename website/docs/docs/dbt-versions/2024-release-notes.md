...
<Expandable alt_header="Coalesce 2024 announcements">
  ...
  - [Python SDK](/docs/dbt-platform-apis/sl-python) is now generally available 
...
- **Behavior change:** User API tokens have been deprecated. Update to [personal access tokens](/docs/dbt-platform-apis/user-tokens) if you have any still in use.
...
- **New**: The [<Constant name="semantic_layer" /> Python software development kit](/docs/dbt-platform-apis/sl-python) is now [generally available](/docs/dbt-versions/product-lifecycles). It provides users with easy access to the <Constant name="semantic_layer" /> with Python and enables developers to interact with the <Constant name="semantic_layer" /> APIs to query metrics/dimensions in downstream tools.
...
- **Enhancement**: The [dbt Semantic Layer Python SDK](/docs/dbt-platform-apis/sl-python) now supports `TimeGranularity` custom grain for metrics. This feature allows you to define custom time granularities for metrics, such as `fiscal_year` or `retail_month`, to query data using non-standard time periods.
...
- **New**: We now expose the `meta` field within the [config property](/reference/resource-configs/meta) for dbt Semantic Layer metrics in the [JDBC and GraphQL APIs](/docs/dbt-platform-apis/sl-api-overview) under the `meta` field.
...
- **Enhancement:** In [Google Sheets](/docs/platform-integrations/semantic-layer/gsheets), you can now apply granularity to all time dimensions, not just metric time. This update uses our [APIs](/docs/dbt-platform-apis/sl-api-overview) to support granularity selection on any chosen time dimension.
...
- **Enhancement:** The [GraphQL API](/docs/dbt-platform-apis/sl-graphql) includes a `requiresMetricTime` parameter to better handle metrics that must be grouped by time. (Certain metrics defined in MetricFlow can't be looked at without a time dimension).
...
- **New:** You can now query the [saved queries](/docs/build/saved-queries) you've defined in the <Constant name="semantic_layer" /> using [Tableau](/docs/platform-integrations/semantic-layer/tableau), [GraphQL API](/docs/dbt-platform-apis/sl-graphql), [JDBC API](/docs/dbt-platform-apis/sl-jdbc), and the [<Constant name="dbt" /> CLI](/docs/platform/cloud-cli-installation). 
...
- **Enhancement:** The YAML spec parameter `label` is now available for Semantic Layer metrics in [JDBC and GraphQL APIs](/docs/dbt-platform-apis/sl-api-overview). This means you can conveniently use `label` as a display name for your metrics when exposing them.
...
- **Enhancement:** Added support to expose `expr` and `agg` for [Measures](/docs/build/measures) in the [GraphQL API](/docs/dbt-platform-apis/sl-graphql).
