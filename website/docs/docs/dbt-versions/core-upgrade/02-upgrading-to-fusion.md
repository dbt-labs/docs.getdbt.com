...
If you pass `--models` as the value to `-s` (for example, `dbt run -s --models`), Fusion's stricter CLI arg parsing will error because no valid selector value was supplied to `-s`. dbt Core silently treated `--models` as a model name in this case. Update any scripts or job definitions using this pattern to pass a valid selector value.
