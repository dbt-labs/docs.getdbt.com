...
- As part of the [dbt Semantic layer](/docs/use-dbt-semantic-layer/dbt-sl) re-launch (in beta), the spec for `metrics` has changed significantly. Refer to the [migration guide](/docs/build/latest-metrics-spec) for more info on how to migrate to the re-launched dbt Semantic Layer.
