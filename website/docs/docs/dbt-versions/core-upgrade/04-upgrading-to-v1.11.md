...
- The new `datasharing` profile credential migrates metadata queries from PostgreSQL catalog tables (for example, `pg_*`, `information_schema`) to Redshift's native `SHOW` commands (for example, `SHOW TABLES`, `SHOW COLUMNS`, `SHOW SCHEMAS`). This enables cross-database and cross-cluster access with [Redshift Datasharing](https://docs.aws.amazon.com/redshift/latest/dg/datashare-overview.html). Refer to [Redshift setup](/docs/local/connect-data-platform/redshift-setup) for configuration details.
