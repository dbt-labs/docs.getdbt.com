...
- (Introduced, disabled by default) [`require_sql_header_in_test_configs`](/reference/global-configs/behavior-changes#sql_header-in-test-configs). When set to `True`, you can set [`sql_header`](/reference/resource-configs/sql_header) in the `config` of a generic data test at the model level or column level in your properties YAML file. For more information, refer to [Data test configurations](/reference/data-test-configs).
