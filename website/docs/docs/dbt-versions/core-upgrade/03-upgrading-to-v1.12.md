...
- You can use the [`refresh_warehouse`](/reference/resource-configs/snowflake-configs#refresh-warehouse) parameter to specify a separate warehouse for a dynamic table's automatic refresh operations, independent of `snowflake_warehouse` which is used for <Term id="ddl" />  execution. This lets you use a smaller warehouse for refreshes while keeping a larger warehouse for DDL.
