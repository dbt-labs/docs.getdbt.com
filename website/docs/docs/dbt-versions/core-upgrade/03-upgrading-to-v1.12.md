...
- dbt now surfaces occasional, non-blocking [CLI hints](/reference/global-configs/hints) that suggest ways to speed up your project. Hints are enabled by default. Disable them with `--no-hints-enabled` or `hints_enabled: false` in `dbt_project.yml`.
