...
- (Mature, enabled by default) [`skip_nodes_if_on_run_start_fails`](/reference/global-configs/behavior-flag-maturity#skip_nodes_if_on_run_start_fails). When `true`, dbt skips all selected resources if an `on-run-start` hook fails.
- (Mature, enabled by default) [`state_modified_compare_more_unrendered_values`](/reference/global-configs/behavior-flag-maturity#state_modified_compare_more_unrendered_values). When `true`, dbt uses unrendered values for `state:modified` checks to reduce false positives across environments.
- (Mature, enabled by default) [`require_yaml_configuration_for_mf_time_spines`](/reference/global-configs/behavior-flag-maturity#require_yaml_configuration_for_mf_time_spines). When `true`, dbt requires MetricFlow time spine configuration to use YAML rather than a SQL config block.
- (Mature, enabled by default) [`require_batched_execution_for_custom_microbatch_strategy`](/reference/global-configs/behavior-flag-maturity#require_batched_execution_for_custom_microbatch_strategy). When `true`, dbt executes custom microbatch strategies in batches. Only relevant if your project has a custom microbatch macro.
- (Mature, enabled by default) [`require_nested_cumulative_type_params`](/reference/global-configs/behavior-flag-maturity#require_nested_cumulative_type_params). When `true`, dbt raises an error (instead of a warning) when cumulative metric type params are not nested under `cumulative_type_params`.
- (Mature, enabled by default) [`validate_macro_args`](/reference/global-configs/behavior-flag-maturity#validate_macro_args). When `true`, dbt validates macro argument names and types during parsing and raises warnings for mismatches.
- (Mature, enabled by default) [`require_all_warnings_handled_by_warn_error`](/reference/global-configs/behavior-flag-maturity#require_all_warnings_handled_by_warn_error). When `true`, all warnings are routed through the `--warn-error` / `--warn-error-options` handler. Projects using `--warn-error` may see new build failures on previously ignored warnings.
