...
- **Enhancement**: You can now view logs for [external metadata ingestion](/docs/explore/external-metadata-ingestion#viewing-ingestion-run-logs) runs in the Run history dashboard. Click on individual run steps to open the log drawer and view detailed information about the metadata ingestion process. This provides better visibility into artifact ingestion and helps troubleshoot any issues that occur during ingestion.
