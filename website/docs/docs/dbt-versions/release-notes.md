...
## July 2024
- **New**: Introduced Semantic validations in CI pipelines. Automatically test your semantic nodes (metrics, semantic models, and saved queries) during code reviews by adding warehouse validation checks in your CI job with the new `dbt sl validate --select state:modified+` command. This ensures validation of modified semantic nodes and guarantees that any code changes made to dbt models don't break these metrics. Refer to [Semantic validations in CI](/docs/deploy/ci-jobs#semantic-validations-in-ci) for more information and use cases.
