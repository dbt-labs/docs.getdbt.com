...
- **Enhancement:** The environment [Connection profiles](/docs/cloud/about-profiles#environment-profiles-table) page has been updated: The profile name is now a clickable button that opens the view/edit drawer (in both view and edit mode), the Connection column links to the connection details page in a new tab, and in edit mode a swap icon button lets you change the assigned profile. The previous ellipsis menu has been removed. For details, see [About profiles](/docs/cloud/about-profiles).
