...
- **Enhancement**: The <Constant name="platform_cli" /> now automatically handles package dependencies when you run dbt commands. You no longer need to manually run `dbt deps` when starting your environment. The CLI automatically installs dependencies from your `packages.yml` file before executing commands. You should still run `dbt deps` manually only when you make changes to your `packages.yml` file. For more information, see [Install dbt CLI](/docs/cloud/cloud-cli-installation).
