...
- **New**: <Constant name="core"/> now loads environment variables from a `.env` file in your current working directory when running commands locally. Shell environment variables take precedence over `.env` values. New projects created with `dbt init` include `.env` in the default `.gitignore`. For more information, refer to [.env file](/reference/dbt-jinja-functions/env_var#env-file).
