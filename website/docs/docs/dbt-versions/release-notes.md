...
- **Fix**: Improved run details pages for Fusion-eligible accounts during assisted upgrade flows. Added support for `fusion-compatible`, `fusion-extended`, and `fusion-fallback`; these options now appear in version dropdowns and run filters based on account feature availability.
