...
## April 2026
- **Enhancement:** Catalog now supports `function` as a resource type. You can browse, search, and filter UDFs from the sidebar and resource search, and view function details (arguments, return type, and source code) on their own detail page. Functions appear in their own mini-lineage view but are not shown in the main project DAG. For more information, refer to [user-defined functions](/docs/build/udfs).
