...
- **Behavior change:** New projects in trial, starter, or Enterprise accounts now default to **Fusion Latest** for new deployment environments when a supported adapter (Redshift, Snowflake, BigQuery, and Databricks) is selected. You can revert to another version by changing the dbt version in your [environment settings](/docs/dbt-cloud-environments#change-environment-settings).
