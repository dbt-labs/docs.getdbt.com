...
- **Enhancement:** The platform metadata credentials setup is now more discoverable. When a connection does not yet have platform metadata credentials configured, the credentials form now opens by default, so you no longer need to click **Add credentials** first. If you cancel setup, the **Add credentials** option still appears so you can reopen the form. Existing connections with configured credentials keep their current edit flow.
