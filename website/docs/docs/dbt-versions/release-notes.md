...
- **New**: When running commands locally, <Constant name="core"/> now loads environment variables from a `.env` file in your current working directory. Shell environment variables take precedence over `.env` values. New projects created with `dbt init` include `.env` in the default `.gitignore`. For more information, refer to [`.env` file](/reference/dbt-jinja-functions/env_var#env-file).
