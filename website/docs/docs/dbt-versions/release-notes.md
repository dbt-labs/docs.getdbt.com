...
- **New**: The [Developer agent](/docs/dbt-ai/developer-agent) is now in beta. Use the Developer agent to write or refactor dbt models from natural language, generate documentation, tests, semantic models, and SQL code from scratch, giving you the flexibility to modify or fix generated code. For more information, see [Developer agent](/docs/dbt-ai/developer-agent).
...
- **Enhancement:** dbt Explorer now supports `function` as a selector resource type, enabling you to filter and select [user-defined functions (UDFs)](/docs/build/udfs) using node selectors. For example, use `dbt build --select "resource_type:function"` to build all functions in your project, or `dbt list --select "resource_type:function"` to list them. This requires dbt v1.11 or later. For more information, see [Node selector methods](/reference/node-selection/methods#resource_type).
