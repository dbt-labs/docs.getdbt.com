...
- **Enhancement:** You can now use arrays as values for keys in the extended attributes YAML editor in <Constant name="dbt_platform" />. For example, `db_groups: [db_editor, db_viewer]` is now valid. Previously, array values were only supported using the API. For more info, refer to [Extended attributes](/docs/dbt-cloud-environments#extended-attributes).
