...
- **New:** Universal login URL is available at [https://login.dbt.com](https://login.dbt.com), making it easier for you to view accounts you have access to across instances (regions and tenancies). After you verify your email, you'll be able to see and access accounts you have access to. This is currently available for multi-tenant accounts with an account-specific domain; support for single-tenant accounts is coming soon. You can still sign in using your **Access URL** directly. Account administrators can enable or disable account discovery with **Enable global account discovery** in [Account settings](/docs/cloud/account-settings#enable-global-account-discovery). For more information, refer to [Log in to dbt platform](/docs/cloud/about-cloud/login).
