...
- **Beta:** Apache Spark is now supported in the <Constant name="fusion_engine" /> CLI. Currently, only Spark 3.0 is supported in <Constant name="fusion" />. For more information, see [Connect Apache Spark to Fusion](/docs/local/connect-data-platform/spark-setup).
