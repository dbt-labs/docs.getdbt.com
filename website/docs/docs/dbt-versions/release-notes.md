...
- **Fix**: Fixed an issue that could cause run details pages to crash for some Fusion-eligible accounts during assisted upgrade flows. Previously, some <Constant name="fusion" /> release-track values were not handled correctly during version comparison. Added support for `fusion-compatible`, `fusion-extended`, and `fusion-fallback`; these options appear in version dropdowns and run filters based on account feature availability.
