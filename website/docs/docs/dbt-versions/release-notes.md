...
- **Beta:** Apache Spark is now supported in the <Constant name="fusion_engine" /> CLI, enabling faster compilation and execution for your Spark-based dbt projects. Currently, <Constant name="fusion" /> only supports Apache Spark 3.0. For more information, see [Connect Apache Spark to Fusion](/docs/local/connect-data-platform/spark-setup).
