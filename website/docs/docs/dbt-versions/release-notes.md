...
- **Enhancement:** [Deferral](/reference/node-selection/defer) now supports [user-defined functions (UDFs)](/docs/build/udfs). When you run with `--defer` and `--state`, `function()` calls resolve to the UDF in the state manifest, so you can run models that depend on UDFs without building those UDFs first. For more information, see [Defer](/reference/node-selection/defer) and [UDFs](/docs/build/udfs).
