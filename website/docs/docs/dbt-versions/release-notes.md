...
- **Enhancement**: [<Constant name="dbt" /> <Constant name="semantic_layer" />](/docs/use-dbt-semantic-layer/dbt-sl) users on single-tenant configurations no longer need to contact their account representative to enable this feature. Setup is now self-service and available across all tenant configurations.
- **New**: The <Constant name="dbt" /> <Constant name="semantic_layer" /> now supports Postgres as a data platform. For more details on how to set up the <Constant name="dbt" /> <Constant name="semantic_layer" /> for Postgres, see [Set up the <Constant name="dbt" /> <Constant name="semantic_layer" />](/docs/use-dbt-semantic-layer/setup-sl).
...
- **New**: <Constant name="dbt" /> <Constant name="copilot" />, an AI-powered assistant, is now generally available in the <Constant name="cloud" /> <Constant name="cloud_ide" /> for all <Constant name="cloud" /> Enterprise accounts. Check out [<Constant name="dbt" /> <Constant name="copilot" />](/docs/cloud/dbt-copilot) for more information.   

#### Also available this month

- **New**: Bringing your own [Azure OpenAI key](/docs/cloud/enable-dbt-copilot#bringing-your-own-openai-api-key-byok) for [<Constant name="dbt" /> <Constant name="copilot" />](/docs/cloud/dbt-copilot) is now generally available. Your organization can configure <Constant name="dbt" /> <Constant name="copilot" /> to use your own Azure OpenAI keys, giving you more control over data governance and billing.
- **New**: The <Constant name="dbt" /> <Constant name="semantic_layer" /> supports Power BI as a [partner integration](/docs/cloud-integrations/avail-sl-integrations), available in private beta. To join the private beta, please reach out to your account representative. Check out the [Power BI](/docs/cloud-integrations/semantic-layer/power-bi) integration for more information.
...
- **Fix**: <Constant name="dbt" /> <Constant name="semantic_layer" /> errors in the Cloud <Constant name="cloud_ide" /> are now displayed with proper formatting, fixing an issue where newlines appeared broken or difficult to read. This fix ensures error messages are more user-friendly and easier to parse.
...
- **New**: You can now query metric alias in <Constant name="dbt" /> <Constant name="semantic_layer" /> [GraphQL](/docs/dbt-cloud-apis/sl-graphql) and [JDBC](/docs/dbt-cloud-apis/sl-jdbc) APIs. 
