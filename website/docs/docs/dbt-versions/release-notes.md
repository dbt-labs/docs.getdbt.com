...
#### dbt Semantic Layer
- **New:** Introducing a new granularity option for cumulative metrics, `period_agg`, nested under the `type_params` parameter. It specifies how to roll up the cumulative metric to another granularity. Options are `start`, `end`, `average`. Defaults to `start` if no `window` is specified. 
- **New:** Added validation that derived metrics always have expr
- **New:** Enabled cache to pickup where filters that are included in saved queries
- **Enhancement:** Cumulative metrics now supports `type_params` under a nested key: `cumulative_type_params`. MetricFlow supports the old `type_params` parameter but will throw a warning if found there.
- **Enhancement:** Updated MetricFlow to use new `cumulative_type_params` fields for cumulative metrics.
...
- **Enhancement:** We now allow granularity for all time dimensions. You can now apply granularity to all time dimensions, not just metric time. This update uses our [APIs](/docs/dbt-cloud-apis/sl-api-overview) to support granularity selection on any chosen time dimension. 
