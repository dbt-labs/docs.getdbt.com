...
- **Enchancement:** Improved `dbt ls` and `dbt list` run log status in <Constant name="dbt_platform" />. In run logs, list-only node results now appear as `noop` instead of `unknown` when no terminal execution status is produced. Refer to [About dbt ls (list) command](/reference/commands/list) for more information.
