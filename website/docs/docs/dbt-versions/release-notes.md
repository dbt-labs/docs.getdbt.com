...
- **Enhancement:** When a model fails a data test, [state-aware orchestration](/docs/deploy/state-aware-about) rebuilds it on subsequent runs instead of reusing it from prior state to ensure quality issues are re-evaluated.
