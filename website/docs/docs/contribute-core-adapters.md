...
- Join both the dedicated channel, [#adapter-ecosystem](https://getdbt.slack.com/archives/C030A0UF5LM), in [dbt Slack](https://community.getdbt.com/) and the channel for your adapter's data store. Refer to the **Slack Channel** link in the [<Constant name="core" /> platform](/docs/local/connect-data-platform/profiles.yml) pages. 
- Check out the open issues in the plugin's source repository. Use the relevant **GitHub repo** link in the [<Constant name="core" /> platform](/docs/local/connect-data-platform/profiles.yml) pages. 
