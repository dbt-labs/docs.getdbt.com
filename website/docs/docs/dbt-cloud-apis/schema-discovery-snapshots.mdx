...
### Fields
The snapshots object can access the _same fields_ as the [Snapshot node](/docs/dbt-cloud-apis/discovery-schema-snapshots). The difference is that the snapshots object can output a list so instead of querying for fields for one specific snapshot, you can query for those parameters for all snapshots within a jobID, database, and so on.
