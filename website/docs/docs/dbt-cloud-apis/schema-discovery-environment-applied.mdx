...
:::info `executionInfo` fields track two different concepts

Within `executionInfo`, `lastRun*` fields reflect the most recent run _attempt_ (regardless of outcome), while `execute*`, `executionTime`, `runGeneratedAt`, and `lastSuccess*` fields reflect the most recent _successful_ materialization. Refer to [Project state](/docs/dbt-cloud-apis/project-state#definition-logical-vs-applied-state-of-dbt-nodes) for more information.
