...
### Fields
Sources has access to the *same fields* as the [Source node](/docs/dbt-cloud-apis/metadata-schema-source).  The difference is that Sources can output a list, so instead of querying for fields for one specific source, you can query for those parameters for all sources within a jobID, database, etc.
