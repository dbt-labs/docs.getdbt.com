...
### Fields
Models has access to the *same fields* as the [Model node](/docs/dbt-cloud-apis/metadata-schema-model).  The difference is that Models can output a list, so instead of querying for fields for one specific model, you can query for those parameters for all models within a jobID, database, etc.
