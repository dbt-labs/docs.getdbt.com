...
### Fields
Snapshots has access to the *same fields* as the [Snapshot node](/docs/dbt-cloud-apis/metadata-schema-snapshots).  The difference is that Snapshots can output a list, so instead of querying for fields for one specific snapshot, you can query for those parameters for all snapshots within a jobID, database, etc.
