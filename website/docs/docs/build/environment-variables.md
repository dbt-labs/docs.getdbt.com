...
Env vars work well with username/password and keypair, including scheduled jobs, because <Constant name="core" /> consumes the Jinja inserted into the autogenerated [`profiles.yml`](/docs/local/profiles.yml) and resolves it to do an `env_var` lookup. 
