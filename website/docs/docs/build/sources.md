...
If you're using <Constant name="fusion_engine" /> with [dbt State](/docs/deploy/dbt-state-about), dbt automatically tracks source freshness using warehouse metadata. You don't need to configure `warn_after` or `error_after` for dbt to detect when source data changes.
