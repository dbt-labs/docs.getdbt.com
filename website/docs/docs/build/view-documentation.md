...
Where you work changes how you view your docs:
...
- Locally, run `dbt docs generate` and then `dbt docs serve`, or host the output on a static file host. `dbt docs serve` only works with the self-hosted dbt CLI and isn't supported in the dbt platform. 
