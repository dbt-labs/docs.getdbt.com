...
SCD Type II tables have a particular dimension with a start and end date. Use the additional [entity `type`](/docs/build/entities#entity-types) parameters, `natural` key, to join tables If an SCD table has a `natural` key as an [entity `type`](/docs/build/entities#entity-types), you don't usually need a `primary` key. In most instances, SCD tables don't have a logically usable `primary` key because `natural` keys map to multiple rows.
