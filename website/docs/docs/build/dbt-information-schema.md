...
This queries the intermediate SQL views directly, without connecting to your warehouse. You don't need to run `--generate-info-schema` first. `--info <view>` is equivalent to `--inline "select * from {{ info_schema('<view>') }}"`.
