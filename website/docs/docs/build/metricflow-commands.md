...
<!--remove when fixed -->
Note: The [**Defer to staging/production** toggle](/docs/cloud/about-cloud-develop-defer) is currently not available when running Semantic Layer commands. To use defer for Semantic Layer commands, manually add the `--defer` flag to the command. This is a temporary workaround and will be available soon.
</TabItem>
