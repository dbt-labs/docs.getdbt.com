...
This section applies to <Constant name="dbt_platform" /> users running the dbt v2 or v1 engine where commands and validations execute remotely in <Constant name="dbt_platform" />.
...
- The following table lists the commands compatible with <Constant name="dbt_platform"/> (<Constant name="studio_ide"/>, VS Code/Cursor, <Constant name="platform_cli" />) powered by the dbt v2 or v1 engine:
