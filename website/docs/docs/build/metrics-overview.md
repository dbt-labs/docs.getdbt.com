...
- **Within semantic models** (under `models` -> `semantic_model` -> `metrics`) for metrics that use measures and dimensions from a single semantic model. Recommended for simple metrics.
