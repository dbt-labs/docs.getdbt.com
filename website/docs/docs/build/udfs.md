...
4. Reference the UDF in a model using the `{{ function(...) }}` macro. For example:
    ...
    When using [`--defer`](/reference/node-selection/defer), `function()` resolves to the UDF definition from the state manifest (for example, production) if the UDF is not selected or not yet built in your target environment. This allows models that depend on UDFs to run successfully in slim CI and development workflows.
