...
DuckDB support for `catalogs.yml` requires the [<Constant name="fusion_engine" />](/docs/fusion/about-fusion) (v2) with the `use_catalogs_v2` behavior flag enabled. It isn't available in the legacy Python `dbt-duckdb` adapter for <Constant name="core" /> v1.
