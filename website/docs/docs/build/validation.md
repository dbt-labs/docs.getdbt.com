...
You can run validations from the <Constant name="dbt_platform" /> or the command line with the following [MetricFlow commands](/docs/build/metricflow-commands). In <Constant name="dbt" />, you need user credentials to run `dbt sl validate` in the IDE or CLI, and deployment credentials to run it in CI.
