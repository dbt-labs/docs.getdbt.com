...
Validations refer to the process of checking whether a system or configuration meets the expected requirements or constraints. In the case of the <Constant name="semantic_layer" />, powered by MetricFlow, there are three built-in validations &mdash; [parsing](#parsing), [semantic](#semantic-syntax), and [data platform](#data-platform).
...
- For <Constant name="core" /> (open source) users or v2 CLI users not connected to dbt platform and using local MetricFlow:
