...
| [`concurrent_batches`](/reference/resource-properties/concurrent_batches) | Overrides dbt's auto detect for running batches concurrently (at the same time). Setting to  `true` runs batches in parallel. Setting to `false` runs batches sequentially (one after the other). Read more about [configuring concurrent batches](/docs/build/incremental-microbatch#configure-concurrent_batches). | `None` | Boolean | Optional |
