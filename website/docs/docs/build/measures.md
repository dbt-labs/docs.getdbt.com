...
Heads up, measures have been deprecated in favor of simple metrics under the `metrics:` key. Migrate by converting each measure to a `type: simple metric`. For more info, check out [Metrics YAML spec in the dbt Fusion engine](/docs/build/metrics-spec-fusion) and [upgrade to dbt Fusion v2.0](/docs/dbt-versions/core-upgrade/upgrading-to-fusion).
:::
