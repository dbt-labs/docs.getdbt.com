...
Heads up, measures have been deprecated in favor of simple metrics under the `metrics:` key. Migrate by converting each measure to a `type: simple` metric. For more info, check out [Migrate to the latest YAML spec](/docs/build/latest-metrics-spec) and [upgrade to v2](/docs/dbt-versions/dbt-upgrade/upgrading-to-v2).
:::
