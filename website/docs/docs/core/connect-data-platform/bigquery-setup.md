...
Starting <Constant name="core" /> v1.11, you can configure dbt to log BigQuery job links at the info level by setting `job_link_info_level_log: true` in your BigQuery profile. This makes the job links visible in standard dbt output, making it easier to access the BigQuery console for debugging and monitoring.
