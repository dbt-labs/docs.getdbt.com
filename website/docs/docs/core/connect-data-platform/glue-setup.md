...
|<Constant name="dbt" /> output database|The database updated by <Constant name="dbt" /> (this is the schema configured in the profile.yml of your <Constant name="dbt" /> environment)|
|<Constant name="dbt" /> source database|All databases used as source|
|<Constant name="dbt" /> output bucket|The bucket name where the data will be generated by <Constant name="dbt" /> (the location configured in the profile.yml of your <Constant name="dbt" /> environment)|
|<Constant name="dbt" /> source bucket|The bucket name of source databases (if they are not managed by Lake Formation)|
...
```bash
$ pip3 install <Constant name="dbt" />-glue
```
...
|-----------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-----------|
| project_name	                           | The <Constant name="dbt" /> project name. This must be the same as the one configured in the <Constant name="dbt" /> project.	                                                                                                                                                                                                            | yes       |
| type	                                   | The driver to use.	                                                                                                                                                                                                                                                                               | yes       |
| query-comment	                          | A string to inject as a comment in each query that <Constant name="dbt" /> runs. 	                                                                                                                                                                                                                                    | no        |
...
| glue_session_id                         | re-use the glue-session to run multiple <Constant name="dbt" /> run commands: set a glue session id you need to use                                                                                                                                                                                                   | no        | 
| glue_session_reuse                      | Reuse the glue-session to run multiple <Constant name="dbt" /> run commands: If set to true, the glue session will not be closed for re-use. If set to false, the session will be closed                                                                                                                             | no        | 
...
<Constant name="dbt" /> seeks to offer useful and intuitive modeling abstractions by means of its built-in configurations and materializations.
...
test_project:
  ...
  outputs:
    dev:
      ...
      session_provisioning_timeout_in_seconds: 120
      location: "s3://aws-<Constant name="dbt" />-glue-datalake-1234567890-eu-west-1/"
...
- Iceberg Tables are natively supported by Athena. Therefore, you can query tables created and operated with <Constant name="dbt" />-glue adapter from Athena.
- Incremental Materialization with Iceberg file format supports <Constant name="dbt" /> snapshot. You are able to run a <Constant name="dbt" /> snapshot command that queries an Iceberg Table and create a <Constant name="dbt" /> fashioned snapshot of it. 
...
test_project:
  ...
  outputs:
    dev:
      ...
      conf: --conf spark.sql.extensions=org.apache.iceberg.spark.extensions.IcebergSparkSessionExtensions --conf spark.serializer=org.apache.spark.serializer.KryoSerializer --conf spark.sql.warehouse=s3://aws-<Constant name="dbt" />-glue-datalake-1234567890-eu-west-1/dbt_test_project --conf spark.sql.catalog.glue_catalog=org.apache.iceberg.spark.SparkCatalog --conf spark.sql.catalog.glue_catalog.catalog-impl=org.apache.iceberg.aws.glue.GlueCatalog --conf spark.sql.catalog.glue_catalog.io-impl=org.apache.iceberg.aws.s3.S3FileIO --conf spark.sql.catalog.glue_catalog.lock-impl=org.apache.iceberg.aws.dynamodb.DynamoDbLockManager --conf spark.sql.catalog.glue_catalog.lock.table=myGlueLockTable  --conf spark.sql.extensions=org.apache.iceberg.spark.extensions.IcebergSparkSessionExtensions 
```
...
test_project:
  ...
  outputs:
    dev:
      ...
      session_provisioning_timeout_in_seconds: 120
      location: "s3://aws-<Constant name="dbt" />-glue-datalake-1234567890-eu-west-1/"
      default_arguments: "--enable-metrics=true, --enable-continuous-cloudwatch-log=true, --enable-continuous-log-filter=true, --enable-spark-ui=true, --spark-event-logs-path=s3://bucket-to-write-sparkui-logs/<Constant name="dbt" />/"
```
...
In many cases, you may need to run you <Constant name="dbt" /> jobs to read from another AWS account.
...
For more information, check the <Constant name="dbt" /> documentation about [custom schemas](https://docs.getdbt.com/docs/build/custom-schemas).

## AWS Lakeformation integration
The adapter supports AWS Lake Formation tags management enabling you to associate existing tags defined out of <Constant name="dbt" />-glue to database objects built by <Constant name="dbt" />-glue (database, table, view, snapshot, incremental models, seeds).

- You can enable or disable lf-tags management via config, at model and <Constant name="dbt" />-project level (disabled by default)
- If enabled, lf-tags will be updated on every <Constant name="dbt" /> run. There are table level lf-tags configs and column-level lf-tags configs. 
...
- Please note that if the tag you want to associate with the table does not exist, the <Constant name="dbt" />-glue execution will throw an error

The adapter also supports AWS Lakeformation data cell filtering. 
- You can enable or disable data-cell filtering via config, at model and <Constant name="dbt" />-project level (disabled by default)
- If enabled, data_cell_filters will be updated on every <Constant name="dbt" /> run.
...
- By default, if you don't specify any column or excluded_columns, <Constant name="dbt" />-glue does not perform Column level filtering and let the principal access all the columns.
...
#### At model level
This way of defining your Lakeformation rules is appropriate if you want to handle the tagging and filtering policy at object level. Remember that it overrides any configuration defined at <Constant name="dbt" />-project level. 
...
This way you can specify tags and data filtering policy for a particular path in your <Constant name="dbt" /> project (eg. models, seeds, models/model_group1, etc.)
...
For more information, check the <Constant name="dbt" /> documentation about [testing a new adapter](https://docs.getdbt.com/docs/contributing/testing-a-new-adapter).
...
Some <Constant name="dbt" /> features, available on the core adapters, are not yet supported on Glue:
