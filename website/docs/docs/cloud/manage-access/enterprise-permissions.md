...
By default, all users can perform Fusion upgrades. When the Fusion upgrade permissions setting is enabled, only users with the fusion admin or account admin permission set can perform upgrades. If the setting is disabled, upgrades are not restricted.
