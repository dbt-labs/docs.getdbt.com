...
Do not use vanity URLs when configuring the SSO settings. Use the generic URL for your environment provided in the SSO settings. For example, if your vanity URL is `cloud.MY_COMPANY.getdbt.com` use `auth.cloud.getdbt.com` as `<YOUR_AUTH0_URI`>`.
