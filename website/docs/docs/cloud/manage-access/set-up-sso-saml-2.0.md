...
3. Provide the following SSO details:
   ...
   | X.509&nbsp;Certificate | Paste the **X.509 Certificate** shown in the IdP setup instructions<br />**Note:** If the IdP does not include the certificate within its response payload, the certificate must be base64 encoded here.<br />**Note:** When the certificate expires, an Idp admin will have to generate a new one to be pasted into dbt Cloud for uninterrupted application access. |
