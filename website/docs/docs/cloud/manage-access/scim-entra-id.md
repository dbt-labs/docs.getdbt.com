...
SCIM-native license mapping (using a SCIM attribute, as supported with Okta) isn't available for Microsoft Entra ID. However, you can still use [SSO-based AD group → license mapping](/docs/cloud/manage-access/seats-and-users#mapped-configuration) alongside an active Entra ID SCIM setup.

Whe you use the SSO-based AD group → license mapping setup, keep the **Manage user licenses with SCIM** (found in **Account settings -> SSO & SCIM**) toggle disabled. Turning it on tells <Constant name="dbt_platform" /> to ignore existing SSO license mappings, which would remove license mapping entirely for Entra ID users.
