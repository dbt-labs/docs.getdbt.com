...
Connect <Constant name="cloud" /> to Amazon's Athena interactive query service to build your <Constant name="dbt" /> project. The following are the required and optional fields for configuring the Athena connection:
...
| AWS S3 staging directory      | s3_staging_dir   | S3 location to store Athena query results and metadata                              | String | Required  | s3://bucket/<Constant name="dbt" />/ |
...
| AWS S3 data directory         | s3_data_dir      | Prefix for storing tables, if different from the connection's s3_staging_dir        | String | Optional  | s3://bucket2/<Constant name="dbt" />/ |
...
| AWS S3 temp tables prefix     | s3_tmp_table_dir | Prefix for storing temporary tables, if different from the connection's s3_data_dir | String | Optional  | s3://bucket3/<Constant name="dbt" />/ |
...
| Schema                | schema                | Specify the schema (Athena database) to build models into (lowercase only) | String | Required | <Constant name="dbt" /> |
