...
You can connect to BigQuery using WIF with external OAuth, including Microsoft Entra ID-backed flows, for [Semantic Layer](/docs/use-dbt-semantic-layer/dbt-sl) queries. For WIF–authenticated BigQuery connections, the connection processing gateway automatically refreshes federated access tokens on common expiration or authentication failures and recycles pooled connections to help reduce authentication outages. 
