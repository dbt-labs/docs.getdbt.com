...
This section describes the different authentication methods for connecting dbt Cloud to Snowflake. You configure Deployment environment (Production, Staging, General) credentials globally in the [**Connections**](/docs/deploy/deploy-environments#deployment-connection) area of your **Account settings**. Individual users configure their development credentials in the **Credentials** section of their user profile [**Credentials**](/docs/cloud/dbt-cloud-ide/develop-in-the-cloud#get-started-with-the-cloud-ide). 
