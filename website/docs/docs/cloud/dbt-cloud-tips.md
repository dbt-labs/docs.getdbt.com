...
- Command-P/Command-Shift-P or Control-P to see the command palette
...
- Press Command-Option-forward slash (Cmd + Opt + /) or Control-Alt-forward slash on the selected code to add a block comment. SQL files will use the Jinja syntax `({# #})` rather than the SQL one `(/* */)`. Markdown files will use the Markdown syntax `(<!-- -->)`
