...
- <Constant name="copilot" /> is optimized for OpenAI's `gpt-3.x`, `o`, `.1-[mini|nano]`, and `gpt-4.5` (deprecated by OpenAI) models, and requires access to the `text-embedding-3-small` embedding model. Other models, like `o1` and `o2`, are not supported and will not work with <Constant name="copilot"/>.
