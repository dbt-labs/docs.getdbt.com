...
In <Constant name="cloud" />, you can import a git repository from any valid git URL that points to a <Constant name="dbt" /> project. There are some important considerations to keep in mind when doing this.
...
- After adding this key, <Constant name="cloud" /> will be able to read and write files in your <Constant name="dbt" /> project.
...
<Constant name="cloud" /> can work with <Constant name="dbt" /> projects hosted on AWS CodeCommit, but there are some extra steps needed compared to Github or other git providers. This guide will help you connect your CodeCommit-hosted <Constant name="dbt" /> project to <Constant name="cloud" />.
...
- **[Contact <Constant name="dbt" /> Support](mailto:support@getdbt.com)** and share that field so that <Constant name="dbt" /> Support team can complete the setup process for you.
...
🎉 **You're all set!** Once <Constant name="dbt" /> Support handles your request and you've set your custom branch, your project is ready to execute <Constant name="dbt" /> runs on <Constant name="cloud" />.
...
Don't see your git provider here? Please [contact <Constant name="dbt" /> Support](mailto:support@getdbt.com) - we're happy to help you set up <Constant name="cloud" /> with any supported git provider.
