...
To use our native integration with Azure DevOps in dbt Cloud, an account admin needs to set up an Microsoft Entra ID (Entra ID) app. We recommend setting up a separate [Entra ID application than used for SSO](/docs/cloud/manage-access/set-up-sso-azure-active-directory).

1. [Register an Entra ID app](#register-an-azure-ad-app).
...
Once the Entra ID app is added to dbt Cloud, an account admin must also connect a service user via OAuth, which will be used to power headless actions in dbt Cloud such as deployment runs and CI.
...
Many customers ask why they need to select Multitenant instead of Single tenant, and they frequently get this step wrong. Microsoft considers Azure DevOps (formerly called Visual Studio) and Microsoft Entra ID as separate tenants, and in order for this Active Directory application to work properly, you must select Multitenant.
...
You also need to add another redirect URI to your Entra ID application. This redirect URI will be used to authenticate the service user for headless actions in deployment environments.
...
4. Complete the form:
    ...
    - **Client Secrets:** You need to first create a secret in the Entra ID App under **Client credentials**. Make sure to copy the **Value** field in the Entra ID App and paste it in the **Client Secret** field in dbt Cloud. You are responsible for the Entra ID app secret expiration and rotation.
...
3. The admin will be directed to Azure DevOps and must accept the Entra ID app's permissions.
