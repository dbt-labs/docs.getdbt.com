...
- [ ] **Review behavior change flag overrides:** <Constant name="fusion" /> removes a subset of [behavior change flags](/reference/global-configs/behavior-changes#behavior-change-flags) (see the **Removed** column) and always enables their new behavior — remove any `flags:` overrides that opt out of those. Other flags remain configurable (including Fusion-specific flags on that page); validate that your project works with the mature / removed defaults before upgrading.
