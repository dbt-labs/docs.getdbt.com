...
- [**Self-hosted only**](#dbt-local-installations) &mdash; Use local tools like the [dbt VS Code extension](/docs/about-dbt-extension) or [install dbt](/docs/local/install-dbt) locally, to develop and run dbt on your own infrastructure. You can use local tools with or without a <Constant name="dbt_platform" /> account.
