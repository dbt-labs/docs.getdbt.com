...
#### Overriding timeout for Python models

For Python models running on Dataproc (cluster or serverless) or BigFrames, use the `timeout` config key in your model config. The `timeout` value is passed directly to the Dataproc or BigFrames job submission and overrides the profile-level `job_execution_timeout_seconds`.
