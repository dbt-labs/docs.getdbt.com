...
| **Preview and Compile button** | You can run your code against your data platform by clicking the **Preview** or **Compile** button in the IDE. By default, the IDE will display 500 rows, but you can change this by adding a `limit your_number` command. For instance, `SELECT * FROM table limit 100` will return up to 100 rows. |
