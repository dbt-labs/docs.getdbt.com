...
- Leverage the speed and scale of the <Constant name="fusion_engine" /> or use <Constant name="core" />:
  ...
  - [Install dbt <Constant name="fusion" /> CLI](/docs/local/install-dbt?version=2) &mdash; Provides <Constant name="fusion" /> performance benefits (faster parsing, compilation, execution) but does not include <Term id="lsp"/> features.
