...
- `get_lineage`:  Gets complete lineage (ancestors/descendants) for dbt resources in a single call. Supports name-based search for models, sources, seeds, and snapshots. Returns max 50 nodes per direction with pagination metadata.
