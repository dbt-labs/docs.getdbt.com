...
The MCP server provides access to the <Constant name="platform_cli"/>, [API](/docs/dbt-platform-apis/overview), the [Discovery API](/docs/dbt-platform-apis/discovery-api), and [Semantic Layer](/docs/use-dbt-semantic-layer/dbt-sl). It provides access to private APIs, text-to-SQL, and SQL execution.
...
Note that access to the [dbt APIs](/docs/dbt-platform-apis/overview) is limited depending on your [plan type](https://www.getdbt.com/pricing).
