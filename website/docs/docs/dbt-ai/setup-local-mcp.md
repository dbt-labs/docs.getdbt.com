...
| `DBT_TOKEN` | Required | Your personal access token or service token from the <Constant name="dbt_platform" />. <br/>**Note**: If you're using a service token, assign the permission sets for the toolsets you enable &mdash; `Semantic Layer Only` for <Constant name="semantic_layer" /> tools and `Metadata Only` for Discovery tools. The self-hosted server doesn't check permissions when it starts; each API authorizes the token when a tool calls it, so a token covering only some toolsets still works for those toolsets, and calls to the others fail individually. This differs from [remote MCP](/docs/dbt-ai/setup-remote-mcp#token-based-authentication), where a token is checked once at connection. <br/><br/> The `execute_sql` tool requires a [Personal Access Token (PAT)](/docs/dbt-apis/user-tokens) — service tokens do not work for this tool. For <Constant name="semantic_layer" /> use, a PAT is also recommended. |
