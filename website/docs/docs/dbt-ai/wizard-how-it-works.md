...
<Constant name="wizard" /> can connect to MCP servers from the CLI, including the [dbt MCP server](/docs/dbt-ai/about-mcp), for access to platform APIs, <Constant name="semantic_layer" /> metadata, and cross-project context. For the full setup (like supported server types, configuration keys, authentication, and examples), refer to [Use MCP servers with <Constant name="wizard" />](/docs/dbt-ai/wizard-mcp).
