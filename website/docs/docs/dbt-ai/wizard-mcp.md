...
You can configure external MCP servers only in the <Constant name="wizard" /> CLI. You can't add your own MCP servers in the <Constant name="dbt_platform" /> (<Constant name="studio_ide" /> and the home app). When the CLI detects a dbt project, it automatically starts its internal `dbt_index` tools for local project metadata and dbt commands. The separate dbt MCP server and the [dbt Agent Skills collection](https://github.com/dbt-labs/dbt-agent-skills) aren't preinstalled; add them when you need their additional capabilities.
:::
