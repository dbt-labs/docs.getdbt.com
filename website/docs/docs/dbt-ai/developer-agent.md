...
After you run a command, <Constant name="copilot" /> adds an icon and **Run by Copilot** tooltip to the command in the <Constant name="studio_ide" /> [**Commands** tab](/docs/platform/studio-ide/ide-user-interface#console-section). This helps you distinguish agent-run commands from commands you ran manually and inspect the run results and logs.
