...
A way to double check after SSO has been set up would be going to this [url](https://cloud.getdbt.com/api/v2/whoami/) and assuring the `auth_provider_domain` listed is the same as the one you configured. Remember to update the domain name if you are configured with a single-tenant cloud or on-premise installation.
