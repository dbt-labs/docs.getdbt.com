...
Currently, authorization of requests takes place [using a service token](/docs/dbt-cloud/dbt-cloud-api/service-tokens). dbt Cloud admin users can generate a Metadata Only service token that is authorized to execute a specific query against the Metadata API.
