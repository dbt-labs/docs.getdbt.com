...
The <Term id="model" /> object allows you to query information about a particular model in a given job.
...
The example query below uses the `parentsModels` and `parentsSources` fields to fetch information about a model’s parent <Term id="model">models</Term> and parent sources. Note that we put a placeholder jobID and uniqueID, which you will have to replace.
