...
The <Term id="model">models</Term> object allows you to query information about all models in a given job.
...
- Find a specific <Term id="model" /> by providing `<database>.<schema>.<identifier>`
...
### Fields
Models has access to the *same fields* as the [Model node](/docs/dbt-cloud/dbt-cloud-api/metadata/schema/metadata-schema-model).  The difference is that Models can output a list, so instead of querying for fields for one specific model, you can query for those parameters for all models within a jobID, database, etc.
