---
title: "Models"
id: "metadata-schema-models"
---

import { ArgsTable, SchemaTable } from "./schema";


The models object allows you to query information about many models in a given job.
...
### Fields
Models has access to the *same fields* as the [Model node](metadata-schema-model).  The difference is that Models can output a list, so instead of querying for fields for one specific model, you can query for those parameters for all models within a jobID, database, etc.
