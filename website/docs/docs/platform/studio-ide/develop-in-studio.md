...
<DetailsToggle alt_header="What is the difference between developing on the Studio IDE, the dbt platform CLI, and dbt?">
You can develop dbt using the web-based IDE in the <Constant name="dbt_platform" /> or on the command line interface using the <Constant name="platform_cli" /> or open-source <Constant name="dbt" />, all of which enable you to execute dbt commands. The key distinction between the <Constant name="platform_cli" /> and <Constant name="dbt" /> is the <Constant name="platform_cli" /> is tailored for the <Constant name="dbt_platform" />'s infrastructure and integrates with all its features:
...
- <Constant name="dbt" />: <Constant name="dbt" /> is an <a href="https://github.com/dbt-labs/dbt">open-sourced</a> software that's freely available. You can build your dbt project in a code editor, and run dbt commands from the command line
