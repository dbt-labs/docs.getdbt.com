...
- **On-premises GitHub**: On Enterprise and Enterprise+ plans, you can use the native integration by registering a [custom GitHub application](#custom-github-application). On other plans, [connect using a Git URL](/docs/platform/git/import-a-project-by-git-url), which supports [fewer](/docs/platform/git/import-a-project-by-git-url#limited-integration) Git features.
  - **Single-tenant accounts**: You have additional connection options that support all native integration features, including CI triggers. Contact dbt Labs support or your account team to
    coordinate any required networking setup with the dbt Labs infrastructure team. Learn more about [single-tenant accounts](/docs/platform/about-platform/tenancy#single-tenant).
