...
- Can access <Constant name="catalog" />.
- Includes `user_credential_write`, so users can view and edit their own development credentials on **Your profile** > **Credentials** without access to the <Constant name="studio_ide" /> or <Constant name="platform_cli" />.
