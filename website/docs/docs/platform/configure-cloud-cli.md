...
5. You should now be able to [use the <Constant name="platform_cli" />](#use-the-dbt-cloud-cli) and run [dbt commands](/reference/dbt-commands) like [`dbt environment show`](/reference/commands/dbt-environment) to view your <Constant name="dbt" /> configuration details or `dbt compile` to compile models in your dbt project.
