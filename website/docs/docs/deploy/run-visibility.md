...
When logs are truncated, a notice appears at the top of the log. Because only the tail is displayed, a model that is still running may not appear in the logs until it completes and its output reaches the tail. This behavior applies to both console and debug logs.
