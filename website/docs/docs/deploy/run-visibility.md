...
<Constant name="cloud" /> developers can access their run history for the last 365 days through the <Constant name="cloud" /> user interface (UI) and API.

<Constant name="dbt" /> Labs limits self-service retrieval of run history metadata to 365 days to improve <Constant name="cloud" />'s performance.
...
You can view or download in-progress and historical logs for your <Constant name="dbt" /> runs. This makes it easier for the team to debug errors more efficiently.
...
The **Compare** tab is shown for [CI job runs](/docs/deploy/ci-jobs) with the **Run compare changes** setting enabled. It displays details about [the changes from the comparison <Constant name="dbt" /> performed](/docs/deploy/advanced-ci#compare-changes) between what's in your production environment and the pull request. To help you better visualize the differences, <Constant name="cloud" /> highlights changes to your models in red (deletions) and green (inserts).
