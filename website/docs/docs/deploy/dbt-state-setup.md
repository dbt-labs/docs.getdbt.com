...
When [dbt State](/docs/deploy/dbt-state-about) is enabled, `dbt login` is used for dbt State authentication. Running this command opens a browser window with two options:
