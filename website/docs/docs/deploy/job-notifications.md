...
- **Is canceled** option &mdash; A job run is canceled.
  - Slack notifications may not be sent for runs canceled by the platform's inactivity-timeout cleanup process (which cancels runs that have been inactive for 10 minutes). To receive notifications for inactivity-timeout cancellations, use email notifications or webhooks.
