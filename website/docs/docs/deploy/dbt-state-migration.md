...
- **No concurrent build detection**: In state-aware orchestration, if a model was being built by one job, any other job that included the same model would skip it. dbt State doesn't have this mechanism; if two jobs include the same model and run at the same time (for example, an hourly and a daily job that overlap), both jobs build the model independently. This results in duplicate compute and, for snapshots and incremental models, can cause data corruption. 
