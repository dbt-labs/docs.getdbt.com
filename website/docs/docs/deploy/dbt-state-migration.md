...
- **No concurrent build detection**: In most cases, dbt State handles overlapping jobs the same way as state-aware orchestration: Job 1 builds the model, and Job 2 sees that the model has already been built and reuses it. However, if both jobs reach the same model at the same time, before Job 1 records in state that the model is complete, both jobs can build the model independently. For snapshots and incremental models, this can lead to data corruption.

  To prevent duplicate builds, ensure jobs that share the same models don't run at the same time &mdash; whether by configuring job schedules through the [<Constant name="dbt_platform" /> job scheduler](/docs/deploy/job-scheduler) or an external orchestration tool (for example, Airflow or Dagster), or by avoiding manually triggered runs that overlap.
