...
- **Model-level freshness threshold**: The [`lag_tolerance`](/reference/resource-configs/lag-tolerance) config sets how stale upstream data can be before dbt triggers a rebuild &mdash; acting as a compute-saving buffer that helps you meet data freshness Service Level Agreements (SLAs) without unnecessary rebuilds.
