...
:::tip More features
<Constant name="dbt" /> Labs plans to provide additional Advanced CI features in the near future. More info coming soon.
...
For [CI jobs](/docs/deploy/ci-jobs) that have the [**<Constant name="dbt" /> compare** option enabled](/docs/deploy/ci-jobs#set-up-ci-jobs), <Constant name="cloud" /> compares the changes between the last applied state of the production environment (defaulting to deferral for lower compute costs) and the latest changes from the pull request, whenever a pull request is opened or new commits are pushed.  

<Constant name="dbt" /> reports the comparison differences in:
...
- The selected data is cached on <Constant name="dbt" /> Labs' systems for up to 30 days. No data is retained on <Constant name="dbt" /> Labs' systems beyond this period.
- The cache is encrypted and stored in an Amazon S3 or Azure blob storage in your account’s region. 
- <Constant name="dbt" /> Labs will not access cached data from Advanced CI for its benefit and the data is only used to provide services as directed by you. 
...
If using dynamic data masking in the data warehouse, the cached data will no longer be dynamically masked in the Advanced CI output, depending on the permissions of the users who view it. <Constant name="dbt" /> Labs recommends limiting user access to unmasked data or considering using synthetic data for the Advanced CI testing functionality.
