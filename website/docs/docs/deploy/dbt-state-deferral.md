...
- **<Constant name="dbt_platform" />**: To defer to an environment other than the default (for example, staging), add `defer-env-id` to the `dbt-cloud` block in `dbt_project.yml`. Refer to [Configure the dbt platform CLI](/docs/platform/dbt-cli-installation) for more information.
