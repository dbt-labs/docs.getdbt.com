...
- Your production and development [environments](/docs/dbt-platform-environments) are on <Constant name="dbt" />’s **v1 Latest** [release track](/docs/dbt-versions/dbt-release-tracks) or a supported dbt version.
...
- (Optional) To enable [Language Server Protocol (LSP) features](/docs/explore/navigate-dbt-insights#lsp-features-in-dbt-insights) in <Constant name="insights" /> and run your compilations on the <Constant name="fusion_engine" />, set your development environment to use the **v2 Stable** dbt version.
