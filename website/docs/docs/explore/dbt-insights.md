...
- (Optional) To enable [Language Server Protocol (LSP) features](/docs/explore/navigate-dbt-insights#lsp-features-in-dbt-insights) in <Constant name="insights" /> and run your compilations on the <Constant name="fusion_engine" />, set your development environment to use the **<Constant name="fusion" /> Stable** dbt version.
