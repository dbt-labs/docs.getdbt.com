...
<Constant name="catalog" /> is available on dbt Starter and all Enterprise plans. Certain features within <Constant name="catalog" /> are only available on Enterprise and Enterprise+ plans, including Model Performance, project recommendations, multi-project lineage, column-level lineage, and more. Refer to the [feature availability by plan](/docs/explore/explore-projects#feature-availability-by-plan) table for a complete list.
