...
When <Constant name="dbt" /> runs, it creates a directed acyclic graph (DAG) of links between models. The number of threads represents the maximum number of paths through the graph <Constant name="dbt" /> may work on at once – increasing the number of threads can minimize the run time of your project.
...
* Increasing the number of threads increases the load on your warehouse, which may impact other tools in your data stack. For example, if your BI tool uses the same compute resources as <Constant name="dbt" />, their queries may get queued during a <Constant name="dbt" /> run.
