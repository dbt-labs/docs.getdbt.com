...
This beta capability requires deployment jobs on [Latest](/docs/dbt-versions/dbt-release-tracks) with <Constant name="core" /> and _isn't_ available on [<Constant name="fusion" /> Stable](/docs/dbt-versions/dbt-release-tracks). For setup details, refer to [Orchestrate exposures](/docs/platform-integrations/orchestrate-exposures).
