...
- [`dbt docs generate`](/reference/commands/cmd-docs) also compiles your project by default (unless you pass [`--no-compile`](/reference/commands/cmd-docs)), so introspective macros such as [`run_query`](/reference/dbt-jinja-functions/run_query) can still hit the warehouse during documentation builds. Refer to [`run_query`](/reference/dbt-jinja-functions/run_query) for **`flags.WHICH`** and guidance on avoiding unintended DML or other side effects when you only intended to generate docs.
