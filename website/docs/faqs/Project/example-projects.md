...
* **Quickstart Tutorial:** You can build your own example <Constant name="dbt" /> project in the [quickstart guide](/docs/get-started-dbt)
...
* **GitLab:** Gitlab's internal <Constant name="dbt" /> project is open source and is a great example of how to use <Constant name="dbt" /> at scale ([source code](https://gitlab.com/gitlab-data/analytics/-/tree/master/transform/snowflake-dbt))
* **dummy-<Constant name="dbt" />:** A containerized <Constant name="dbt" /> project that populates the Sakila database in Postgres and populates <Constant name="dbt" /> seeds, models, snapshots, and tests. The project can be used for testing and experimentation purposes ([source code](https://github.com/gmyrianthous/dbt-dummy))
