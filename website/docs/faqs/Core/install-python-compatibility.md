...
Use this table to match <Constant name="core" /> versions with their compatible Python versions. New [<Constant name="dbt" /> minor versions](/docs/dbt-versions/core#minor-versions) will add support for new Python3 minor versions when all dependencies can support it. In addition, <Constant name="dbt" /> minor versions will withdraw support for old Python3 minor versions before their [end of life](https://endoflife.date/python).
