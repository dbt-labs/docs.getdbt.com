...
| For projects that connect to a data platform, [install <Constant name="core" /> with an adapter](/docs/core/pip-install#installing-the-adapter) | `python -m pip install dbt-core dbt-ADAPTER_NAME` <br /> (For example, `python -m pip install dbt-core dbt-snowflake`) | Provides a complete, compatible, and ready-to-run dbt setup |
