...
When you connect Snowflake to <Constant name="dbt_platform"/> using [OAuth](/docs/platform/manage-access/set-up-snowflake-oauth), dbt stores a refresh token. This allows your user credentials to remain usable in tools like the <Constant name="studio_ide" /> and the dbt Semantic Layer without needing to re-authenticate each time.
