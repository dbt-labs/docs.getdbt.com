...
dbt Core is the software that takes a [dbt project](dbt-project) (`.sql` and `.yml` files) and a command and then creates tables/views in your warehouse. dbt Core includes a command line interface (CLI) so that users can execute dbt commands using a terminal program. dbt Core is open source and free to use.
