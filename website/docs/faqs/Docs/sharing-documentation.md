...
Access <Constant name="dbt" /> <Constant name="explorer" /> in <Constant name="cloud" /> by clicking the **Explore** link in the navigation. You can have up to 5 read-only users access the documentation for your project.

<Constant name="cloud" /> developer plan and <Constant name="core" /> users can use [<Constant name="dbt" /> Docs](/docs/collaborate/build-and-view-your-docs#dbt-docs), which generates basic documentation but it doesn't offer the same speed, metadata, or visibility as <Constant name="dbt" /> <Constant name="explorer" />. 
