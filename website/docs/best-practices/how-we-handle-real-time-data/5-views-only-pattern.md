...
This page uses Snowflake for code examples, but the views-only pattern can be adapted to other warehouses.
:::
...
- A non‑dbt system (ETL, streaming pipeline, app) is already writing into a warehouse‑resident table such as `raw.realtime_events` or `ops.active_sessions`, and that table meets your SLA for latency.
...
- Maximum freshness &mdash; The view reflects new data as soon as it lands in `raw.realtime_events`.
- Simple operations &mdash; No incremental logic to tune and no extra dbt job needed just to keep the data fresh. You still schedule jobs for tests, docs, and so on.
- Best for small datasets &mdash; Works well when tables are small and queries are simple. Computing the view on the fly is cheap and fast.
...
- [Incremental models](/best-practices/how-we-handle-real-time-data/2-incremental-patterns), or
...
If the continuously‑updated source is itself a live operational store (not a warehouse landing table), you must be careful not to overload it with analytics queries. In most cases, we recommend:

- Replicating into a warehouse table first (Snowflake, BigQuery, Databricks, and so on), then
- Applying this views‑only pattern within the warehouse, not directly on the OLTP system.
