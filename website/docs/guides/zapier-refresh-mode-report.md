...
description: Use Zapier to trigger a Mode dashboard refresh when a dbt job completes.
hoverSnippet: Learn how to use Zapier to trigger a Mode dashboard refresh when a dbt job completes.
...
## Store secrets 
In the next step, you will need the Webhook Secret Key from the prior step, and a <Constant name="dbt" /> [personal access token](/docs/dbt-platform-apis/user-tokens) or [service account token](/docs/dbt-platform-apis/service-tokens), as well as a [Mode API token and secret](https://mode.com/developer/api-reference/authentication/). 
