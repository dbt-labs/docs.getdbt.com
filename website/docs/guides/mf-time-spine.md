...
To support custom calendars (like fiscal years, fiscal quarters, and so on), create an additional time spine and configure it in YAML. This feature is available in the <Constant name="dbt" />'s [**v1 Latest** release track](/docs/dbt-versions/dbt-release-tracks) or [<Constant name="core" /> 1.9 and later](/docs/dbt-versions/dbt-upgrade/upgrading-to-v1.9).
