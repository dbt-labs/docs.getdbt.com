...
- The database where the object is created is defined by the database configured at the [environment level in <Constant name="cloud" />](/docs/dbt-cloud-environments) or in the [`profiles.yml` file](/docs/local/connect-data-platform/profiles.yml) in dbt Core.
