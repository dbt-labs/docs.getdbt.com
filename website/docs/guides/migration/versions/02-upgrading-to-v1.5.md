...
Some options that could previously be specified before a sub-command now can only be specified afterwards. e.g. `dbt --profiles-dir . run` is no longer valid and must be `dbt run --profiles-dir .` instead.
