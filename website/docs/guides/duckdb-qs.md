...
- [Create a virtual development environment](/docs/core/pip-install#using-virtual-environments) using a template provided by <Constant name="dbt" /> Labs.
- We will set up a fully functional <Constant name="dbt" /> environment with an operational and executable project. The codespace automatically connects to the DuckDB database and loads a year's worth of data from our fictional Jaffle Shop café, which sells food and beverages in several US cities.
...
- Run any <Constant name="dbt" /> command from the environment’s terminal. 
- Generate a larger dataset for the Jaffle Shop café (for example, five years of data instead of just one).

You can learn more through high-quality [<Constant name="dbt" /> Learn courses and workshops](https://learn.getdbt.com). 
...
- When using DuckDB with <Constant name="core" />, you'll need to use the <Constant name="dbt" /> command-line interface (CLI). Currently, DuckDB is not supported in <Constant name="cloud" />.
...
4. Ensure your profile is setup correctly from the command line by running the following [<Constant name="dbt" /> commands](/reference/dbt-commands).


    - [<Constant name="dbt" /> compile](/reference/commands/compile) &mdash; generates executable SQL from your project source files
    - [<Constant name="dbt" /> run](https://docs.getdbt.com/reference/commands/run) &mdash; compiles and runs your project
    - [<Constant name="dbt" /> test](https://docs.getdbt.com/reference/commands/test) &mdash; compiles and tests your project
    - [<Constant name="dbt" /> build](https://docs.getdbt.com/reference/commands/build) &mdash; compiles, runs, and tests your project
    - [<Constant name="dbt" /> docs generate](/reference/commands/cmd-docs#dbt-docs-generate) &mdash; generates your project's documentation.
    - [<Constant name="dbt" /> docs serve](/reference/commands/cmd-docs#dbt-docs-serve) &mdash; starts a webserver on port 8080 to serve your documentation locally and opens the documentation site in your default browser.

For complete details, refer to the [<Constant name="dbt" /> command reference](/reference/dbt-commands).
...
1. At the terminal's prompt, you can execute any <Constant name="dbt" /> command you want. For example:
    ...
    For complete information, refer to the [<Constant name="dbt" /> command reference](https://docs.getdbt.com/reference/dbt-commands). Common commands are:
...
As you increase the number of years, it takes exponentially more time to generate the data because the Jaffle Shop stores grow in size and number. For a good balance of data size and time to build, <Constant name="dbt" /> Labs suggests a maximum of 6 years.
## Next steps

Now that you have <Constant name="core" />, DuckDB, and the Jaffle Shop data up and running, you can explore <Constant name="dbt" />'s capabilities. Refer to these materials to get a better understanding of <Constant name="dbt" /> projects and commands:

- The [About projects](/docs/build/projects) page guides you through the structure of a <Constant name="dbt" /> project and its components.
- [<Constant name="dbt" /> command reference](/reference/dbt-commands) explains the various commands available and what they do.
- [<Constant name="dbt" /> Labs courses](https://courses.getdbt.com/collections) offer a variety of beginner, intermediate, and advanced learning modules designed to help you become a <Constant name="dbt" /> expert. 
- Once you see the potential of <Constant name="dbt" /> and what it can do for your organization, sign up for a free trial of [<Constant name="cloud" />](https://www.getdbt.com/signup). It's the fastest and easiest way to deploy <Constant name="dbt" /> today!
...
To document your <Constant name="dbt" /> projects with DuckDB, follow the steps:
...
3. Use a scheduler such [Prefect](/docs/deploy/deployment-tools#prefect) to schedule your <Constant name="dbt" /> runs. You can create a DAG (Directed Acyclic Graph) that triggers <Constant name="dbt" /> commands at specified intervals.
