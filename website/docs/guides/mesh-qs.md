...
<Constant name="dbt" /> <Constant name="mesh" /> is a framework that helps organizations scale their teams and data assets effectively. It promotes governance best practices and breaks large projects into manageable sections &mdash; for faster data development. <Constant name="dbt" /> <Constant name="mesh" /> is available for [<Constant name="cloud" /> Enterprise](https://www.getdbt.com/) accounts.

This guide will teach you how to set up a multi-project design using foundational concepts of [<Constant name="dbt" /> <Constant name="mesh" />](https://www.getdbt.com/blog/what-is-data-mesh-the-definition-and-importance-of-data-mesh) and how to implement a data mesh in <Constant name="cloud" />:
...
- [Deciding how to structure your <Constant name="dbt" /> <Constant name="mesh" />](https://docs.getdbt.com/best-practices/how-we-mesh/mesh-3-structures)
- [<Constant name="dbt" /> <Constant name="mesh" /> best practices guide](https://docs.getdbt.com/best-practices/how-we-mesh/mesh-4-implementation)
- [<Constant name="dbt" /> <Constant name="mesh" /> FAQs](https://docs.getdbt.com/best-practices/how-we-mesh/mesh-5-faqs)

## Prerequisites​

To leverage <Constant name="dbt" /> <Constant name="mesh" />, you need the following:
...
<Constant name="cloud" /> enables data practitioners to develop in their tool of choice and comes equipped with a local [<Constant name="cloud" /> CLI](/docs/cloud/cloud-cli-installation) or in-browser [<Constant name="cloud" /> <Constant name="cloud_ide" />](/docs/cloud/dbt-cloud-ide/develop-in-the-cloud).

In this section of the guide, you will set the "Jaffle | Data Analytics" project as your foundational project using the <Constant name="cloud" /> <Constant name="cloud_ide" />.
...
- Run a [deployment job](/docs/deploy/deploy-jobs) successfully
  - Note, Enable the **Generate docs on run** toggle for this job to update the <Constant name="dbt" /> <Constant name="explorer" />. Once run, you can click Explore from the upper menu bar and see your lineage, tests, and documentation coming through successfully.

## Define a public model and run first job

In the previous section, you've arranged your basic building blocks, now let's integrate <Constant name="dbt" /> <Constant name="mesh" />.
...
2. Navigate to the <Constant name="cloud" /> <Constant name="cloud_ide" /> **Lineage** tab to see the model noted as **Public**, below the model name.
...
3. In the <Constant name="cloud" /> <Constant name="cloud_ide" />, go to **Version control** to commit and merge the changes.
...
Use [<Constant name="dbt" /> <Constant name="explorer" />](/docs/collaborate/explore-projects) to view the lineage across projects in <Constant name="cloud" />. Navigate to the **Explore** page for each of your projects &mdash; you should now view the [lineage seamlessly across projects](/docs/collaborate/explore-multiple-projects).
...
Congratulations 🎉! You're ready to bring the benefits of <Constant name="dbt" /> <Constant name="mesh" /> to your organization. You've learned:
...
- [<Constant name="dbt" /> <Constant name="mesh" /> FAQs](https://docs.getdbt.com/best-practices/how-we-mesh/mesh-5-faqs)
- [Implement <Constant name="dbt" /> <Constant name="mesh" /> with the <Constant name="semantic_layer" />](/docs/use-dbt-semantic-layer/sl-faqs#how-can-i-implement-dbt-mesh-with-the-dbt-semantic-layer)
