...
- **<Constant name="dbt" /> APIs:** Use <Constant name="dbt" />'s robust [APIs](/docs/dbt-platform-apis/overview) to create, read, update, and delete (CRUD) projects/jobs/environments project. The [<Constant name="dbt" /> Administrative API](/docs/dbt-platform-apis/admin-cloud-api) and [Terraform provider](https://registry.terraform.io/providers/dbt-labs/dbtcloud/latest/docs/resources/job) facilitate programmatic access and configuration storage. While the [Discovery API](/docs/dbt-platform-apis/discovery-api) offers extensive metadata querying capabilities, such as job data, model configurations, usage, and overall project health. <Lifecycle status="self_service,managed" />
