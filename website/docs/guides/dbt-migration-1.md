---
title: 'Move to the dbt platform: Get started'
id: dbt-migration-1
description: "Learn how to move to the dbt platform and what you need to get started."
hoverSnippet: "Learn how to move to the dbt platform."
icon: 'guides'
...
This guide outlines the steps you need to take to move from self-hosted dbt to the <Constant name="dbt_platform" /> and highlights the necessary technical changes:

- [Account setup](/guides/dbt-migration-1?step=4): Learn how to create a <Constant name="dbt_platform" /> account, invite team members, and configure it for your team.
- [Data platform setup](/guides/dbt-migration-1?step=5): Find out about connecting your data platform to the <Constant name="dbt_platform" />.
- [<Constant name="git" /> setup](/guides/dbt-migration-1?step=6): Learn to link your dbt project's <Constant name="git" /> repository with the <Constant name="dbt_platform" />.
- [Developer setup:](/guides/dbt-migration-1?step=7) Understand the setup needed for developing in the <Constant name="dbt_platform" />.
- [Environment variables](/guides/dbt-migration-1?step=8): Discover how to manage environment variables in the <Constant name="dbt_platform" />, including their priority.
- [Orchestration setup](/guides/dbt-migration-1?step=9): Learn how to prepare your <Constant name="dbt_platform" /> environment and jobs for orchestration.
- [Models configuration](/guides/dbt-migration-1?step=10): Get insights on validating and running your models in the <Constant name="dbt_platform" />, using either the <Constant name="studio_ide" /> or <Constant name="dbt_platform" /> CLI.
...
- You have an existing <Constant name="dbt" /> project connected to a <Constant name="git" /> repository and data platform supported in the [<Constant name="dbt_platform" />](/docs/platform/connect-data-platform/about-connections). 
- You have a <Constant name="dbt_platform" /> account. **[Don't have one? Start your free trial today](https://www.getdbt.com/signup)**!

## Account setup

This section outlines the steps to set up your <Constant name="dbt_platform" /> account and configure it for your team.

1. [Create your <Constant name="dbt_platform" /> account](https://www.getdbt.com/signup).

2. Provide user [access](/docs/platform/manage-access/about-user-access) and [invite users](/docs/platform/manage-access/about-user-access) to your <Constant name="dbt_platform" /> account and project.
...
This section outlines the considerations and methods to connect your data platform to the <Constant name="dbt_platform" />.

1. In the <Constant name="dbt_platform" />, set up your [data platform connections](/docs/platform/connect-data-platform/about-connections) and [environment variables](/docs/build/environment-variables). The <Constant name="dbt_platform" /> can connect with a variety of data platform providers including:
...
Your existing dbt project source code should live in a <Constant name="git" /> repository. In this section, you will connect your existing dbt project source code from <Constant name="git" /> to the <Constant name="dbt_platform" />.

1. Ensure your dbt project is in a <Constant name="git" /> repository.

2. In **Account settings**, select **Integrations** to [connect your <Constant name="git" /> repository](/docs/platform/git/configure-git) to the <Constant name="dbt_platform" />:
   - (**Recommended**) Connect with one of the [native integrations](/docs/platform/git/configure-git) in the <Constant name="dbt_platform" /> (such as GitHub, GitLab, and Azure DevOps).
...
The most common data environments are production, staging, and development. The way self-hosted dbt manages [environments](/docs/environments-in-dbt) is through `target`, which are different sets of connection details. 

[<Constant name="dbt_platform" /> environments](/docs/dbt-platform-environments) go further by:
...
2. **<Constant name="dbt" /> version** &mdash; In your <Constant name="dbt_platform" /> environment, select a [release track](/docs/dbt-versions/dbt-release-tracks) for ongoing dbt version upgrades. If your team plans to use both self-hosted dbt and the <Constant name="dbt_platform" /> for developing or deploying your dbt project, you can run `dbt --version` in the command line to find out which version of <Constant name="dbt" /> you're using.
   - When using self-hosted dbt, you need to think about which version you're using and manage your own upgrades. When using the <Constant name="dbt_platform" />, leverage [release tracks](/docs/dbt-versions/dbt-release-tracks) so you don't have to.

3. **Connect to your data platform** &mdash; When using the <Constant name="dbt_platform" />, you can [connect to your data platform](/docs/platform/connect-data-platform/about-connections) directly in the UI.
...
### dbt commands
1. Review the [dbt commands](/reference/dbt-commands) supported for <Constant name="dbt_platform" /> development. For example, `dbt init` isn't needed in the <Constant name="dbt_platform" /> as you can create a new project directly in the <Constant name="dbt_platform" />.
...
### Environment variables in dbt
  - <Constant name="dbt_platform" /> environment variables must be prefixed with `DBT_` (including `DBT_ENV_CUSTOM_ENV_` or `DBT_ENV_SECRET`).
  - If your self-hosted dbt environment variables don't follow this naming convention, perform a ["find and replace"](/docs/platform/studio-ide/develop-in-studio#studio-ide-features) in your project to make sure all references to these environment variables contain the proper naming conventions.
- The <Constant name="dbt_platform" /> secures environment variables that enable more flexible configuration of data warehouse connections or git provider integrations, offering additional measures for sensitive values, such as prefixing keys with `DBT_ENV_SECRET`to obscure them in logs and the UI.
...
Environment variables in the <Constant name="dbt_platform" /> are managed with a clear [order of precedence](/docs/build/environment-variables#setting-and-overriding-environment-variables), allowing users to define values at four levels (highest to lowest order of precedence):
...
### dbt environments
To use the [<Constant name="dbt_platform" />'s job scheduler](/docs/deploy/job-scheduler), set up one environment as the production environment. This is the [deployment](/docs/deploy/deploy-environments) environment. You can set up multiple environments for different stages of your deployment pipeline, such as development, staging/QA, and production.

### Initial setup steps
1. **<Constant name="dbt" /> version** &mdash; In your environment settings, configure the <Constant name="dbt_platform" /> with the same <Constant name="dbt" /> version.
...
2. **Configure your jobs** &mdash; [Create jobs](/docs/deploy/deploy-jobs#create-and-schedule-jobs) for scheduled or event-driven dbt jobs. You can use cron execution, manual, pull requests, or trigger on the completion of another job.
   - Note that alongside [jobs in the <Constant name="dbt_platform" />](/docs/deploy/jobs), discover other ways to schedule and run your dbt jobs with the help of other tools. Refer to [Integrate with other tools](/docs/deploy/deployment-tools) for more information.

### Additional configuration
Explore these additional configurations to optimize your <Constant name="dbt_platform" /> orchestration setup further:

1. **Custom target names** &mdash; Use environment variables to set a `custom target.name` for every [corresponding <Constant name="dbt_platform" /> job](/docs/build/custom-target-names) at the environment level.

2. **dbt commands** &mdash; Add any relevant [dbt commands](/docs/deploy/job-commands) to execute your <Constant name="dbt_platform" /> jobs runs.
...
5. **API access** &mdash; Create [API auth tokens](/docs/dbt-apis/authentication) and access to [<Constant name="dbt_platform" /> APIs](/docs/dbt-apis/overview) as needed.  <Lifecycle status="self_service,managed" />
...
Building a custom solution to efficiently check code upon pull requests is complicated. With the <Constant name="dbt_platform" />, you can enable [continuous integration / continuous deployment (CI/CD)](/docs/deploy/continuous-integration) and configure the <Constant name="dbt_platform" /> to run your dbt projects in a temporary schema when new commits are pushed to open pull requests.
...
1. Set up an integration with a native <Constant name="git" /> application (such as Azure DevOps, GitHub, GitLab) and a CI environment in the <Constant name="dbt_platform" />.
...
1. In your [development tool](/docs/platform/about-develop-dbt) of choice, you can review your dbt project, ensure it's set up correctly, and run some [dbt commands](/reference/dbt-commands):
   ...
   - Run a few models in the <Constant name="studio_ide" /> or <Constant name="dbt_platform" /> CLI to ensure you're experiencing accurate results in development.
...
Congratulations on completing the first part of your move to the <Constant name="dbt_platform" /> 🎉!

You have learned:
- How to set up your <Constant name="dbt_platform" /> account
