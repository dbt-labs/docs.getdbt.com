...
- Use [Extended Attributes](/docs/deploy/deploy-environments#extended-attributes) to set a flexible [profiles.yml](/docs/local/connect-data-platform/profiles.yml) snippet in your <Constant name="cloud" /> environment settings. It gives you more control over environments (both deployment and development) and extends how <Constant name="cloud" /> connects to the data platform within a given environment.
...
<Constant name="cloud" /> provides robust orchestration that enables you to schedule, run, and monitor dbt jobs with ease. Here are some tips and caveats to consider when using <Constant name="cloud" />'s orchestration features:
