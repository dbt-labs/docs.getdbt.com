...
### Environment variables in dbt Cloud
  - dbt Cloud environment variables must be prefixed with `DBT_` (including `DBT_ENV_CUSTOM_ENV_` or <VersionBlock lastVersion="1.7">`DBT_ENV_SECRET_`</VersionBlock><VersionBlock firstVersion="1.8">`DBT_ENV_SECRET`</VersionBlock>).
