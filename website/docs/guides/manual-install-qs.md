...
When you use <Constant name="core" /> to work with <Constant name="dbt" />, you will be editing files locally using a code editor, and running projects using a command line interface (CLI). 

If you want to edit files and run projects using the web-based <Constant name="dbt" /> Integrated Development Environment (<Constant name="cloud_ide" />), refer to the [<Constant name="cloud" /> quickstarts](/guides). You can also develop and run <Constant name="dbt" /> commands using the [<Constant name="cloud" /> CLI](/docs/cloud/cloud-cli-installation) &mdash; a <Constant name="cloud" /> powered command line.
...
After setting up BigQuery to work with <Constant name="dbt" />, you are ready to create a starter project with example models, before building your own models.
...
To create your <Constant name="dbt" /> project:
...
```shell
$ pwd
> Users/BBaggins/<Constant name="dbt" />-tutorial/jaffle_shop
```
...
1. Link the GitHub repository you created to your <Constant name="dbt" /> project by running the following commands in Terminal. Make sure you use the correct git URL for your repository, which you should have saved from step 5 in [Create a repository](https://docs.getdbt.com/guides/manual-install?step=2).

```shell
git init
git branch -M main
git add .
git commit -m "Create a <Constant name="dbt" /> project"
git remote add origin https://github.com/USERNAME/<Constant name="dbt" />-tutorial.git
...
In the next steps, you will take a sample query and turn it into a model in your <Constant name="dbt" /> project.
...
We recommend using <Constant name="cloud" /> as the easiest and most reliable way to [deploy jobs](/docs/deploy/deployments) and automate your <Constant name="dbt" /> project in production. 
...
For more information about using <Constant name="core" /> to schedule a job, refer [<Constant name="dbt" /> airflow](/blog/dbt-airflow-spiritual-alignment) blog post.
