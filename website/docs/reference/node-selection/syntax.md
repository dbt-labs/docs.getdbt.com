...
Note that when you're using `--selector`, dbt ignores `--select` and `--exclude`. Starting in <Constant name="core" /> v1.12, dbt raises a warning when `--selector` is combined with `--select` or `--exclude`.
