...
This syntax for defining foreign keys uses `ref`, meaning it will capture dependencies and works across different environments. It's available in [<Constant name="dbt" /> **Latest**](/docs/dbt-versions/dbt-release-tracks) and [<Constant name="dbt" /> v1.9+](/docs/dbt-versions/dbt-upgrade/upgrading-to-v1.9).
