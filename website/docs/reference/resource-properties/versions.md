...
- Follow a consistent naming convention for model versions and aliases.
- Use `defined_in` and `alias` only if you have good reason.
- Use the [`latest_version_pointer`](/reference/resource-configs/latest_version_pointer) config to automatically create a view pointing to the latest version of your model. Enable it per model or globally with the [`latest_version_pointer_enabled_by_default`](/docs/mesh/govern/model-versions#pointing-to-the-latest-version) flag in `dbt_project.yml`.
