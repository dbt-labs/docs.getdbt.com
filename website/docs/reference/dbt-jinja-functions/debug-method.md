...
:::warning Requires dbt CLI

The `debug()` macro is only available when using the self-hosted dbt CLI in a development environment. It's _not available_ in <Constant name="dbt_platform" />. 
