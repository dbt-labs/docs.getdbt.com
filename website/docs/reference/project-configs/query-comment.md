...
## Definition
A string to inject as a comment in each query that <Constant name="dbt" /> runs against your database. This comment can attribute SQL statements to specific <Constant name="dbt" /> resources like models and tests.
...
```sql
/* {"app": "<Constant name="dbt" />", "dbt_version": "1.5.0rc2", "profile_name": "debug",
...
```sql
/* run by drew in <Constant name="dbt" /> */
...
```sql
select ...
/* {"app": "<Constant name="dbt" />", "dbt_version": "1.6.0rc2", "profile_name": "debug", "target_name": "dev", "node_id": "model.dbt2.my_model"} */
;
```
...
The following example shows a JSON query comment which can be parsed to understand the performance characteristics of your <Constant name="dbt" /> project.
...
| ---------------- | ----------- |
| dbt_version      | The version of <Constant name="dbt" /> being used. For details about release versioning, refer to [Versioning](/reference/commands/version#versioning). |
| env_var          | See [env_var](/reference/dbt-jinja-functions/env_var) |
| modules          | See [modules](/reference/dbt-jinja-functions/modules) |
| run_started_at   | When the <Constant name="dbt" /> invocation began |
| invocation_id    | A unique ID for the <Constant name="dbt" /> invocation |
...
| connection_name  | A string representing the internal name for the connection. This string is generated by <Constant name="dbt" />. |
