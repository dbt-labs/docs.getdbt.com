...
1. Defined in YAML files using the `config` [resource property](/reference/model-properties), typically in your [snapshots directory](/reference/project-configs/snapshot-paths) or whichever folder you pefer. Available in [the <Constant name="dbt" /> release track](/docs/dbt-versions/dbt-release-tracks), dbt v1.9 and higher.
