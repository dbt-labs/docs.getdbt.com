...
By using the semantic manifest produced by dbt, MetricFlow will instantiate a data flow plan and generate SQL from <Constant name="semantic_layer" /> query requests. It's a valuable reference that you can use to understand the structure and details of your data models.
...
Starting in <Constant name="dbt" /> v1.12, dbt also writes an `osi_document.json` file to your `target/` directory alongside `semantic_manifest.json` at parse time. This file represents your project's <Constant name="semantic_layer" /> in the [Apache Ossie](https://github.com/apache/ossie) format, a vendor-agnostic schema for describing semantic models and metrics.
