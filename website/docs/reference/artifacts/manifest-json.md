...
_Current schema_: [`v4`](https://schemas.getdbt.com/dbt/manifest/v4/index.html)
...
This single file contains a full representation of your dbt project's resources (<Term id="model">models</Term>, tests, macros, etc), including all node configurations and resource properties. Even if you're only running some models or tests, all resources will appear in the manifest (unless they are disabled) with most of their properties. (A few node properties, such as `compiled_sql`, only appear for executed nodes.)
