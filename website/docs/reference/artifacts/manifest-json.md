...
Today, dbt uses this file to populate the [docs site](/docs/explore/build-and-view-your-docs), and to perform [state comparison](/reference/node-selection/syntax). Members of the community also use it to analyze project health, such as checking for missing descriptions or tests.
...
- `root_path`: Absolute file path of this resource's package. (**Note:** This was removed for most node types in <Constant name="dbt" /> v1.4 / manifest v8 to reduce duplicative information across nodes, but it is still present for seeds.)
