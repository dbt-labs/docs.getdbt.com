...
The `manifest.json` file is generated by any [dbt command](/category/list-of-commands) that parses the project. This includes all commands **except** [`deps`](/reference/commands/deps), [`clean`](/reference/commands/clean), [`debug`](/reference/commands/debug), and [`init`](/reference/commands/init).

After executing a dbt command, the `manifest.json` file can be found in the project's `target/` directory:

- If developing locally &mdash; Open the `target/` directory in your project folder
- In the <Constant name="cloud_ide" /> &mdash; Open the `target/` directory in the file tree
- In <Constant name="dbt_platform" />  jobs &mdash; Download `manifest.json` from the [artifacts](/reference/artifacts/dbt-artifacts) tab for a given job run
