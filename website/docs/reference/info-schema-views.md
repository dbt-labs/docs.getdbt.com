...
You can query the following views in a check. These are available in the [dbt Information Schema](/docs/build/dbt-information-schema) v1 and expose only the columns that are final at parse time. Use these view names in the [`{{ info_schema() }}`](/reference/dbt-jinja-functions/info-schema-macro) macro when writing [checks](/docs/build/checks).
