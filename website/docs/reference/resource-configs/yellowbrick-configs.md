...
The <Constant name="dbt" />-yellowbrick adapter supports the following incremental materialization strategies:

- `append` (default when `unique_key` is not defined)
- `delete+insert` (default when `unique_key` is defined)

All of these strategies are inherited from the <Constant name="dbt" />-postgres adapter.
...
<Constant name="dbt" />-yellowbrick supports the following Yellowbrick-specific features when defining tables:
...
Yellowbrick supports cross-database queries and the <Constant name="dbt" />-yellowbrick adapter will permit cross-database reads into a specific target on the same appliance instance.

## Limitations

This initial implementation of the <Constant name="dbt" /> adapter for Yellowbrick Data Warehouse may not support some use cases. We strongly advise validating all records or transformations resulting from the adapter output.
