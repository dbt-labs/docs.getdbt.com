...
When creating Python UDFs, you can specify third-party Python packages for your Python UDF using the optional `packages` config. List package names (for example, `numpy`, `pandas`); you can pin versions (for example, `pandas==1.5.0`). The warehouse installs these packages when creating the UDF, allowing your UDF to use functionality from external Python libraries.
