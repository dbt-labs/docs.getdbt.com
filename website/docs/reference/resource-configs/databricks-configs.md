...
| use_uniform <sup>‖</sup>   | When `table_format` is `iceberg`, controls whether dbt creates a Unity Catalog managed Iceberg table (`false`) or a UniForm Delta table with Iceberg reads enabled (`true`). Refer to [Databricks Iceberg support](/docs/build/iceberg/adapters/databricks-iceberg-support#choose-between-managed-iceberg-and-uniform). | Optional | SQL, Python | `true` |
