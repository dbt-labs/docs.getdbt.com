...
- If `overwrite_columns` is set, dbt generates SQL that explicitly lists columns in both the `INSERT` target and the `SELECT` projection:
...
- If `overwrite_columns` is not set, dbt currently defaults to `SELECT *`:
