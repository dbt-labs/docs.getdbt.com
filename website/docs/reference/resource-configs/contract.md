...
That said, on certain data platforms, you will need to specify a varchar size or numeric scale, if you do not want it to revert to the default. This is most relevant for the `numeric` type on Snowflake, which defaults to precision of 38 and scale of 0 (zero digits after the decimal, i.e. rounded to an integer). To avoid this implicit coercion, specify your `data_type` with a nonzero scale, e.g. `numeric(38, 6)`.
