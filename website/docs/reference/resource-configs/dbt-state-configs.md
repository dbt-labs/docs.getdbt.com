...
|--------|---------|-------|-------------|
| [`lag_tolerance`](/reference/resource-configs/lag-tolerance) | `45m` | Node, folder, or project-level via model config | How stale a node can get before it's eligible for a rebuild. dbt measures staleness by how much the source data has moved on since the last build — not wall-clock time. Acts as a compute-saving buffer that helps align builds with freshness SLAs. Applies to data freshness only; SQL changes always trigger a rebuild regardless of this setting. |
