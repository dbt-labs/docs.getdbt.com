...
|--------|---------|-------|-------------|
| [`lag_tolerance`](/reference/resource-configs/lag-tolerance) | `45m` | Node, folder, or project-level via model config | How far upstream data can advance beyond the data from a node's last build before it's eligible for a rebuild. Compares source freshness now against source freshness at the last build — not wall-clock time. Acts as a compute-saving buffer that helps align builds with freshness SLAs. Applies to data freshness only; SQL changes always trigger a rebuild regardless of this setting. |
