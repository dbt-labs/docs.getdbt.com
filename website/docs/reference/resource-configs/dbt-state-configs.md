...
|--------|---------|-------|-------------|
| [`lag_tolerance`](/reference/resource-configs/lag-tolerance) | `45m` | Node, folder, or project-level via model config | Defines how fresh upstream data must be before dbt rebuilds. Acts as a compute-saving buffer that helps meet freshness SLAs. Applies to data freshness only; SQL changes always trigger a rebuild regardless of this setting. |
