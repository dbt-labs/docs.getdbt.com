...
|--------|---------|-------|-------------|
| [`lag_tolerance`](/reference/resource-configs/lag-tolerance) | `45m` | Node, folder, or project-level via model config | How much the source data can change before a node is eligible for a rebuild. dbt measures this by how much the source data has moved on since the last build — not wall-clock time. Acts as a compute-saving buffer that helps align builds with freshness SLAs. Applies to data freshness only; SQL changes always trigger a rebuild regardless of this setting. |
