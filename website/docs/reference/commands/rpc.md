...
In the meantime, dbt Labs will be performing critical maintenance only for `dbt-rpc`, until the last compatible version of dbt-core has reached the [end of official support](/docs/dbt-versions/dbt-versions#end-of-life-versions). At that point, dbt Labs will archive this repository to be read-only.
