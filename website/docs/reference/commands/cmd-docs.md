...
With <Constant name="fusion_engine" />, generate docs metadata by running `dbt build --write-catalog` (or `dbtf build --write-catalog`). Metadata is not generated by default with `dbt build` alone. To view it in <Constant name="catalog" />, run a job in <Constant name="dbt_platform" /> so the metadata is uploaded, then open [<Constant name="catalog" />](/docs/explore/build-and-view-your-docs).
:::
...
With <Constant name="fusion_engine" />, [dbt Docs v2](/docs/build/view-documentation#dbt-docs-v2) is the recommended way to generate and view your project's documentation. Use `dbt docs generate` to build the documentation site and `dbt docs serve` to preview it locally.
...
Both write artifacts, but only `dbt docs generate` builds the static dbt Docs site viewable in a browser:

<SimpleTable>
  
| | `--write-catalog` flag | `dbt docs generate` command |
|--|-------------------|---------------------|
| Artifacts | `catalog.json` in addition to any artifacts generated by the command | `index.html`, `assets/`, and the index files |
...
| When it runs | When you add the flag. In <Constant name="dbt_platform" /> jobs on <Constant name="fusion" />, it runs automatically | Only when you run it |
</SimpleTable>
