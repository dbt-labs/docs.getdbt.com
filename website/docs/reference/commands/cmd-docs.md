...
With the <Constant name="fusion_engine" />, generate docs metadata by running `dbtf build --write-catalog`. Metadata is not generated by default with `dbtf build` alone. If you use <Constant name="dbt_platform" />, you can browse that metadata in [<Constant name="catalog" />](/docs/explore/build-and-view-your-docs).
:::
