...
```
<Constant name="dbt" /> show --select "model_name.sql"
```
or
```
<Constant name="dbt" /> show --inline "select * from {{ ref('model_name') }}"
```
...
```bash
$ <Constant name="dbt" /> build -s "my_model_with_duplicates"
13:22:47 .0
...
...
13:22:53 Running with <Constant name="dbt" />=1.5.0
