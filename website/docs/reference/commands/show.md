...
#### `--info` flag
- The `--info <view>` flag queries the [dbt Information Schema](/docs/build/dbt-information-schema) directly from the CLI. It reads from the intermediate views and doesn't connect to your warehouse. You don't need to run `--generate-info-schema` first.
