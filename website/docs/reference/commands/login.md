...
When [dbt State](/docs/deploy/dbt-state-about) is enabled, [`dbt login`](/reference/commands/login?version=2.0#dbt-login---help) is used for dbt State authentication. Running this command opens a browser window where you can log in with your <Constant name="dbt_platform" /> account. Enter your email address. If you don't have a <Constant name="dbt_platform" /> account, dbt Labs will create a standalone [Developer account](https://www.getdbt.com/pricing) for you. After that, you'll authorize access between the CLI and <Constant name="dbt_platform" />.
