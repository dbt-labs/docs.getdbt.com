...
#### SQL compilation error when running the `--empty flag` on model

If you encounter the error: `the error SQL compilation error: syntax error line 1 at position 21 unexpected '('.` when running a model with the `--empty flag`, explicitly call the `.render()` method on that relation.
