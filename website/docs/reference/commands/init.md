...
- Generate a `.gitignore` that includes `.env` (starting <Constant name="core" /> v1.12), `target/`, `dbt_packages/`, and `logs/` so local credentials and build artifacts are excluded from version control by default.
