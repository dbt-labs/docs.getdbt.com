...
- Generate a `.gitignore` that includes `.env`, `target/`, `dbt_packages/`, and `logs/` so local credentials and build artifacts are excluded from version control by default.
