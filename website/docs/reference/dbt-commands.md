...
| [list](/reference/commands/list) | Lists resources defined in a dbt project |  ✅ | All tools <br /> All [supported versions](/docs/dbt-versions) <br /> In <Constant name="dbt_platform" /> run logs for <Constant name="fusion_engine" /> runs, list node results can appear as **No-op** instead of **Unknown** when no terminal status is produced. |
