...
## Overview
Exposures are defined in `.yml` files in your `models` directory (as defined by the [`model-paths` config](model-paths)), nested under an `exposures:` key. You may define `exposures` in YAML files that also define `sources` or `models`.
