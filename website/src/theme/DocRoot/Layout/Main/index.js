  ...
  useEffect(() => {
    ...
    if (!EOLDate) {
      ...
      if (new Date() > new Date(EOLDate)) {
        setEOLData({
          showEOLBanner: true,
          EOLBannerText: `This version of dbt Core is <a href="/docs/dbt-versions/dbt-versions">no longer supported</a>. There will be no more patches or security fixes. For improved performance, security, and features, upgrade to the <a href="https://github.com/dbt-labs/dbt-core/releases/latest"> latest stable version</a>. Some dbt customers might have an extended <a href="/docs/dbt-versions/dbt-versions">critical support window</a>. `,
        });
      } else if (new Date() > threeMonths) {
        setEOLData({
          showEOLBanner: true,
          EOLBannerText: `This version of dbt Core is nearing the end of its <a href="/docs/dbt-versions/dbt-versions">critical support period</a>. For improved perfomance, security, and features, upgrade to the <a href="https://github.com/dbt-labs/dbt-core/releases/latest"> latest stable version</a>.`,
        });
      } else {
        setEOLData({
          showEOLBanner: false,
