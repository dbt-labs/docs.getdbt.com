 ...
 *
 * Outputs four committed partials:
 *   website/docs/docs/dbt-ai/_wizard-cli-generated-v2.md     (dbt wizard, Fusion 2.0+)
 *   website/docs/docs/dbt-ai/_wizard-cli-generated-v1.md     (dbt-wizard, pre-2.0)
 ...
 *
 * The reference pages import these inside <VersionBlock> wrappers.
 *
...
const OUT_V1 = path.join(OUT_DIR, '_wizard-cli-generated-v1.md');
const OUT_V2 = path.join(OUT_DIR, '_wizard-cli-generated-v2.md');
...
function renderMarkdown(displayName, nodes, version) {
  const lines = [];
  lines.push(`<!-- AUTOGENERATED by website/scripts/generate-wizard-cli-reference.js -->`);
  lines.push(`<!-- Source: \`${displayName} --help\` (wizard ${version}) -->`);
  lines.push(`<!-- Do not edit by hand; rerun \`npm run generate:wizard-cli\` in website/ -->`);
  lines.push('');

  // Root flags as "Global flags".
  const root = nodes[0];
  ...
  if (root.usage.length) {
    lines.push('```bash');
    for (const u of root.usage) lines.push(u);
    lines.push('```');
    lines.push('');
  }
  if (root.args.length) {
    lines.push(renderArgsTable(root.args));
  }
  lines.push(renderOptionsTable(root.opts));
  lines.push('');

  // Each subcommand becomes its own ### section.
  for (const node of nodes.slice(1)) {
    const heading = node.pathParts.slice(1).join(' ');
    lines.push(`### \`${displayName} ${heading}\``);
    lines.push('');
    if (node.description) {
    ...
    lines.push(renderOptionsTable(node.opts));
    lines.push('');
  }
...
function stubFile(displayName, reason) {
  return [
    `<!-- AUTOGENERATED by website/scripts/generate-wizard-cli-reference.js -->`,
    `<!-- ${reason} -->`,
    '',
    `:::warning Auto-generated reference unavailable`,
    `The wizard CLI flag reference is generated from \`${displayName} --help\`, but the wizard binary was not found at build time. Install wizard locally and re-run \`npm run generate:wizard-cli\` from \`website/\` to refresh this page.`,
    `:::`,
    '',
  ].join('\n');
}
...
function main() {
  ...
  if (!bin) {
    ...
    const filesExist = [OUT_V1, OUT_V2, OUT_FULL, OUT_SLASH].every(fs.existsSync);
    if (filesExist) {
    ...
    fs.writeFileSync(OUT_V1, stubFile('dbt-wizard', reason));
    fs.writeFileSync(OUT_V2, stubFile('dbt wizard', reason));
    return;
  }
  ...
  const renderFor = (displayName) => {
    const results = [];
    walk(bin, displayName, [path.basename(bin)], '', results, new Set());
    return renderMarkdown(displayName, results, version);
  };

  fs.writeFileSync(OUT_V1, renderFor('dbt-wizard'));
  fs.writeFileSync(OUT_V2, renderFor('dbt wizard'));
  console.log(`[generate-wizard-cli] wrote ${OUT_V1}`);
  console.log(`[generate-wizard-cli] wrote ${OUT_V2}`);
