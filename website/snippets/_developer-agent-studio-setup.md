...
3. Select the [**Agent mode** button](/docs/dbt-ai/developer-agent#agent-modes) to specify the mode for the <Constant name="dev_agent" />. Available modes are **Ask for approval** (default) and **Edit files automatically**.
4. [Review the agent's suggestions](/docs/dbt-ai/developer-agent#reviewing-agent-suggestions) and approve or reject the changes. You can also use the **Start new Copilot chat** button to start a new chat session.
