...
In the [latest YAML spec](/docs/build/latest-metrics-spec), semantic models are defined in model YAML files, and cross-project references are not yet supported. As a result, maintaining a centralized Semantic Layer project that references models across projects is not compatible.
