...
- **`x-dbt-user-id`** — required for `execute_sql`. See [Find your user ID](/faqs/Accounts/find-user-id).
...
`execute_sql` does **not** work with service tokens &mdash; you must use a PAT. For the complete list of headers (including tool-disable options) and the full table, see [Set up remote MCP](/docs/dbt-ai/setup-remote-mcp#token-based-authentication).
