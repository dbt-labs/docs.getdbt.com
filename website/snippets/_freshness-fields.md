| Field | Description |
|---|---|
| `warn_after` | How old the most recent data can be before `dbt freshness` reports a warning. Requires both `count` (positive integer) and `period` (`minute`, `hour`, or `day`). |
