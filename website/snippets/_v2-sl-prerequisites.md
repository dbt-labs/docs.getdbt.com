- Have a <Constant name="dbt" /> Starter, Enterprise, or Enterprise+ account. Available on all [tenant configurations](/docs/platform/about-cloud/tenancy).
...
- Note that the <Constant name="semantic_layer" /> doesn't support using [Single sign-on (SSO)](/docs/platform/manage-access/sso-overview) for [production credentials](/docs/dbt-platform-apis/service-tokens#permissions-for-service-account-tokens), though SSO is supported for development user accounts.
