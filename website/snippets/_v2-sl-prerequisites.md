...
- **Read-only seat users <Lifecycle status="private_beta" />:** Read-only users can set up [personal warehouse credentials](/docs/use-dbt-semantic-layer/setup-sl#configure-development-credentials-and-create-a-personal-token) to query the Semantic Layer using a [personal access token](/docs/dbt-apis/user-tokens). This capability is in private beta as of May 2026. Contact your account manager to join.
