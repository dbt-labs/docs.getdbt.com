...
| Unit test results | [dbt test](/reference/commands/test) or [dbt build](/reference/commands/build) within a job in the environment. Unit tests are typically run in development or CI environments, so their results rarely appear in production <Constant name="explorer" />. |
