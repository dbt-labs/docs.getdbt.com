To change an environment's settings (such as its name, credentials, or dbt version):

1. Navigate to **Orchestration** > **Environments**.
2. Click the **environment name** you want to change.
3. Click **Settings** and then **Edit**.
4. Update the fields requiring updates. For example, you can edit the environment name, dbt version (for eligible account tiers and supported adapters, **Fusion Latest** is the default for new deployment environments).
5. Click **Save** to apply your changes.

For details on available settings, see [Common environment settings](/docs/dbt-cloud-environments#common-environment-settings) in the environment info above.
